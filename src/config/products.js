// Archivo central de productos. Editando este archivo modificarás catálogo, precios e imágenes.
//********************************************************************//
//           ALCANCIAS y COFRES
//********************************************************************//

export const productsConfig = [
  {
    "id": "alcancia-coque",
    "name": "Alcancía Coqueta", 
    "slug": "alcancia-coque",
    "category": "alcancias",
    "price": 25.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "TOP VENTAS",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774224555/goshopping/alcancias/alcancia_coque_pvc.webp"
    ],
    "description": "Ahorrar nunca fue tan tierno. Convierte la disciplina del ahorro en un momento de alegría para grandes y chicos.",
    "specs": [
      {
        "label": "Material",
        "value": "PVC - ABS Premium"
      },
    ],
    "rating": 4.9,
    "reviews": 245,
    "colors": [
      "#0ba9f1ff"
    ],
    "stock": 50,
    "tags": [],
    "fragrances": [],
    "isNew": false,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "alcancia-electrica",
    "name": "Alcancía Eléctrica",
    "slug": "alcancia-electrica",
    "category": "alcancias",
    "price": 29.9,
    "oldPrice": 39.9,
    "currency": "PEN",
    "badge": "-25%",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774224557/goshopping/alcancias/alcancia_elec_cat.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1775166639/alcancia-electrica-modelo_xuxjof.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774224558/goshopping/alcancias/alcancia_elec_panda.webp"
    ],
    "description": "Nostalgia y diversión en un solo lugar. Revive la emoción de las máquinas retro mientras aseguras tu futuro, un diseño clásico que aporta carácter a cualquier espacio.",
    "specs": [
      {
        "label": "Material",
        "value": "ABS + Metal"
      },
      {
        "label": "Funciones",
        "value": "Tacto - Sonido"
      }
    ],
    "rating": 4.3,
    "reviews": 98,
    "colors": [
      "#4D4D4D",
      "#000000"
    ],
    "stock": 18,
    "tags": [],
    "fragrances": [],
    "isNew": false,
    "isOnSale": true,
    "salePercent": 25
  },
  {
    "id": "cofre-telefono",
    "name": "Cofre Telefono ring",
    "slug": "cofre-telefono",
    "category": "alcancias",
    "price": 54.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "OFERTA",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1775167958/cofre-telefono-modelo_qj0vrr.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1775170803/cofre-telefono-corazon-ring-cafe_hnlkl9.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1775170796/cofre-te%C3%B1efono-corazon-ring-blanco_wkaviq.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1775170388/cofre_music_retro_c5woey.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1775170378/cofre_music_rectro_whi_k2hk5i.webp"
    ],
    "description": "Resiliencia y estilo para sus primeros ahorros. Fabricada en PVC de alta resistencia, esta alcancía es el regalo perfecto para enseñar el valor del futuro con un diseño divertido y duradero.",
    "specs": [
      {
        "label": "Material",
        "value": "PVC + Musical"
      }
    ],
    "rating": 4.5,
    "reviews": 88,
    "colors": [
      "#2B0F01",
      "#FFFFFF",
      "#2B0F01",
      "#FFFFFF"
    ],
    "stock": 60,
    "tags": [],
    "fragrances": [],
    "isNew": false,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "alcancia-llorona",
    "name": "Alcancía Llorona",
    "slug": "alcancia-llorona",
    "category": "alcancias",
    "price": 29.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "NUEVO",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774224559/goshopping/alcancias/alcancia_llorona_pvc.webp"
    ],
    "description": "Ahorrar es motivo de alegría. Contagia optimismo a tu espacio mientras protegetus monedas con un diseño icónico que te recordará tus metas con una sonrisa diaria.",
    "specs": [
      {
        "label": "Material",
        "value": "PVC"
      }
    ],
    "rating": 4.7,
    "reviews": 45,
    "colors": [
      "#c2190dff"
    ],
    "stock": 50,
    "tags": [],
    "fragrances": [],
    "isNew": true,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "alcancia-niñas",
    "name": "Alcancía Niñas",
    "slug": "alcancia-niñas",
    "category": "alcancias",
    "price": 29.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "EXCLUSIVO",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774224560/goshopping/alcancias/r0s5sgwcyjkmnd7jwjic.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1775166599/alcancia-ni%C3%B1a-lila_xotuim.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1775166600/alcancia-ni%C3%B1a-rosa_qcqjek.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1775167222/alcancia-ni%C3%B1a-verde_pdb9th.webp"

    ],
    "description": "Expresión y carácter en cada moneda. Un diseño único que captura emociones reales, transformando el ahorro en una pieza de conversación artística para tu hogar.",
    "specs": [
      {
        "label": "Material",
        "value": "PVC Soft Touch"
      }
    ],
    "rating": 4.8,
    "reviews": 32,
    "colors": [
      "#916CEA",
      "#FBB2A3",
      "#90EE90"
    ],
    "stock": 25,
    "tags": [],
    "fragrances": [],
    "isNew": false,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "alcancia-niños",
    "name": "Alcancía Niños",
    "slug": "alcancia-niños",
    "category": "alcancias",
    "price": 29.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "TENDENCIA",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774224562/goshopping/alcancias/bnezvs7qbwlrjpm9d5m9.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1775167225/alcancia-ni%C3%B1o-verde_vwmynz.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1775167942/alcancia-ni%C3%B1o-turquesa_ylpiea.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1775167223/alcancia-ni%C3%B1o-lila_oewayg.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1775166602/alcancia-ni%C3%B1o-beige_yrqggr.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1775166601/alcancia-ni%C3%B1o-azul_ynmsyh.webp"
    ],
    "description": "Protección con actitud. No dejes que nadie toque tus ahorros con esta alcancía de diseño rebelde y sofisticado, ideal para quienes buscan piezas con personalidad propia.",
    "specs": [
      {
        "label": "Material",
        "value": "PVC Resistente"
      }
    ],
    "rating": 4.7,
    "reviews": 28,
    "colors": [
      "#90EE90",
      "#4D4D4D",
      "#916CEA",
      "#FF9800",
      "#003785"
    ],
    "stock": 20,
    "tags": [],
    "fragrances": [],
    "isNew": false,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "alcancia-renegona",
    "name": "Alcancía Renegona",
    "slug": "alcancia-renegona",
    "category": "alcancias",
    "price": 29.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "KIDS",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774224563/goshopping/alcancias/alcancia_renegona_pvc.webp"
    ],
    "description": "Donde empiezan las grandes historias. Fomenta la imaginación y el hábito del ahorro con un diseño encantador que acompañará el crecimiento y los sueños de los más pequeños.",
    "specs": [
      {
        "label": "Material",
        "value": "PVC Ecológico"
      }
    ],
    "rating": 4.9,
    "reviews": 115,
    "colors": [
      "#fffb0dff"
    ],
    "stock": 45,
    "tags": [],
    "fragrances": [],
    "isNew": false,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "alcancia-sonrisa",
    "name": "Alcancía Sonrisa",
    "slug": "alcancia-sonrisa",
    "category": "alcancias",
    "price": 29.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "GIRL",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774224564/goshopping/alcancias/alcancia_sonrisa_pvc.webp"
    ],
    "description": "Dulcedo y seguridad para sus tesoros. Un diseño delicado que aporta una nota de ternura a cualquier habitación infantil, perfecta para guardar momentos y monedas.",
    "specs": [
      {
        "label": "Material",
        "value": "PVC Ecológico"
      }
    ],
    "rating": 4.8,
    "reviews": 94,
    "colors": [
      "#ca0c75ff"
    ],
    "stock": 38,
    "tags": [],
    "fragrances": [],
    "isNew": false,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "cofre-maquina-coser",
    "name": "Cofre Maquina Coser",
    "slug": "cofre-maquina-coser",
    "category": "alcancias",
    "price": 39.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "OFERTA",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1775172862/cofre-maquina-coser-modelo_e5m14k.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1775167947/cofre-maquina-coser_ssryae.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1775167950/cofre-maquina-coser-modelo_ijr0nu.webp"
    ],
    "variants": [
      {
        "name": "Grande",
        "price": 54.9,
        "oldPrice": 59.9
      },
      {
        "name": "Pequeño",
        "price": 39.9,
        "oldPrice": null
      }
    ],
    "description": "Hermoso detalle para regalo. Para esa persona especial que le agrada realizar creatividades",
    "specs": [
      {
        "label": "Material",
        "value": "PVC + Musical"
      }
    ],
    "rating": 4.5,
    "reviews": 88,
    "colors": [
      "#2B0F01",
      "#2B0F01"
    ],
    "stock": 60,
    "tags": [],
    "fragrances": [],
    "isNew": false,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "cofre-toca-disco",
    "name": "Cofre Toca Disco",
    "slug": "cofre-toca-disco",
    "category": "alcancias",
    "price": 54.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "OFERTA",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1775167963/cofre-tocador-modelo_vrxiry.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1775166616/cofre-tocador_gwezfy.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1775170798/cofre-vitrola-blanco_lzfhwi.webp"
    ],
    "description": "Resiliencia y estilo para sus primeros ahorros. Fabricada en PVC de alta resistencia, esta alcancía es el regalo perfecto para enseñar el valor del futuro con un diseño divertido y duradero.",
    "specs": [
      {
        "label": "Material",
        "value": "PVC + Musical"
      }
    ],
    "rating": 4.5,
    "reviews": 88,
    "colors": [
      "#2B0F01",
      "#FFFFFF"
    ],
    "stock": 60,
    "tags": [],
    "fragrances": [],
    "isNew": false,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "cofre-piano",
    "name": "Cofre Piano",
    "slug": "cofre-piano",
    "category": "alcancias",
    "price": 54.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "OFERTA",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1775166613/cofre-piano-modelo_jl4dsi.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1775166611/cofre-piano_facusw.webp"
    ],
    "description": "Resiliencia y estilo para sus primeros ahorros. Fabricada en PVC de alta resistencia, esta alcancía es el regalo perfecto para enseñar el valor del futuro con un diseño divertido y duradero.",
    "specs": [
      {
        "label": "Material",
        "value": "PVC + Musical"
      }
    ],
    "rating": 4.5,
    "reviews": 88,
    "colors": [
      "#FFFFFF",
      "#FFFFFF"
    ],
    "stock": 60,
    "tags": [],
    "fragrances": [],
    "isNew": false,
    "isOnSale": false,
    "salePercent": null
  },

  //********************************************************************//
  //           ANILLOS
  //********************************************************************//

  {
    "id": "anillo-ojos-buho-modelo",
    "name": "Anillo Ojos de Buho",
    "slug": "anillo-ojos-buho-modelo",
    "category": "anillos",
    "price": 10.0,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "LUJO",
    "badgeColor":  null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981288/anillo_ojos_buho_modelo_tnnre0.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981286/anillo_buho_verde_dnoca2.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981286/anillo_buho_azul_l9ibx4.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981286/anillo_buho_naranja_f0lit2.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981285/anillo_buho_rojo_ibqzxm.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981285/anillo_buho_plomo_rqh6fr.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981284/anillo_buho_rosa_fsu595.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981274/anillo_buho_amarillo_qp6ncu.webp"
    ],
    "description": "Diseñado para quienes buscan destacar con un accesorio único, este anillo de ojos de búho combina elegancia y significado. Su acabado en aleación resistente y su diseño detallado hacen que sea una pieza llamativa y duradera.",
    "specs": [
      {
        "label": "Material",
        "value": "Aleación"
      }
    ],
    "rating": 5,
    "reviews": 142,
    "colors": [
      "#90EE90",
      "#003785",
      "#FF9800",
      "#EC1E1B",
      "#D0CCD1",
      "#FBB2A3",
      "#FFFFA2"
    ],
    "stock": 15,
    "tags": [],
    "fragrances": [],
    "isNew": true,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "anillo-pulsera",
    "name": "Anillo Pulsera",
    "slug": "anillo-pulsera",
    "category": "anillos",
    "price": 22.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "NUEVO",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981289/anillo_modelo_entero_cywmwy.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981281/anillo_pulsera_gotico_c3f3lv.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981277/anillo_pulsera_mariposa_l3niih.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981270/anillo_pulsera_escop_plateado_zv9mij.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981269/anillo__duo_scorp_plateado_lpdp13.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981259/anillo_pulsera_escop_dorado_zub0co.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981258/anillo__duo_scorp_dorado_pzyjpa.webp"
    ],
    "description": "Una pieza sofisticada que conecta delicadamente la muñeca con el dedo, creando un efecto visual elegante y único. Ideal para elevar cualquier look con un toque moderno y femenino.",
    "specs": [
      {
        "label": "Material",
        "value": "Aleación"
      }
    ],
    "rating": 4.9,
    "reviews": 38,
    "colors": [
      "#000000",
      "#FFD700",
      "#D0CCD1",
      "#D0CCD1",
      "#FFD700",
      "#FFD700"
    ],
    "stock": 20,
    "tags": [],
    "fragrances": [],
    "isNew": false,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "anillo-ojo-de-dragon",
    "name": "Anillo Ojo Dragon Imperial",
    "slug": "anillo-ojo-de-dragon",
    "category": "anillos",
    "price": 15.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "NUEVO",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981234/anillo_ojos_dragon_modelos_ceszeq.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981252/anillo_ojo_morado_h6eitw.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981273/anillo_ojo_colores_uidcv5.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981240/anillo_ojo_rojo_msuouq.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981239/anillo_ojo_amarillo_st2tzu.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981238/anillo_ojo_azul_hf0nru.webp"
    ],
    "description": "Despierta tu lado más audaz con este impresionante anillo de dragón, una pieza que simboliza fuerza, protección y dominio..",
    "specs": [
      {
        "label": "Material",
        "value": "Aleacion de Hierro"
      }
    ],
    "rating": 4.8,
    "reviews": 45,
    "colors": [
      "#7F00B2",
      "#00FFF6",
      "#EC1E1B",
      "#FFFFA2",
      "#003785"
    ],
    "stock": 25,
    "tags": [],
    "fragrances": [],
    "isNew": true,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "anillo-varon-modelo",
    "name": "Anillo Estilo Men",
    "slug": "anillo-varon-modelo",
    "category": "anillos",
    "price": 16.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "LIDER",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981288/anillo_varon_modelos_gyfjsk.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981280/anillo_oval_scorpion_hormiga_wwizez.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981279/anillo_cuadrado_hormiga_j36is3.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981273/anillo_piedra_celeste_lkw0x1.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981267/anillo_piedra_rojo_aqdjdq.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981266/anillo_piedra_verde_ghx6vr.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981266/anillo_piedra_negra_fpo6ek.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981265/anillo_oval_aleacion_ibaawz.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981260/anillo_cuadrado_aleacion_ddfaxq.webp"

    ],
    "description": "Anillos para hombre con diseños modernos y elegantes.",
    "specs": [
      {
        "label": "Material",
        "value": "Aleacion Zinc/Hierro"
      }
    ],
    "rating": 4.9,
    "reviews": 56,
    "colors": [
      "#FFD700",
      "#D0CCD1",
      "#94E7FF",
      "#EC1E1B",
      "#90EE90",
      "#000000",
      "#D0CCD1",
      "#4D4D4D"
    ],
    "stock": 18,
    "tags": [],
    "fragrances": [],
    "isNew": false,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "anillo-duos",
    "name": "Anillo Dos Piezas",
    "slug": "anillo-duos",
    "category": "anillos",
    "price": 19.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "NUEVO",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981256/anillo_2_piezas_modelo_wfy1zl.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981285/anillo_2_piezas_negro_cuadrado_wpwvua.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981283/anillo_2_piezas_rojo_obsnv3.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981278/anillo_2_piezas_plateado_zv04qp.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981271/anillo_2_piezas_negro_hv6teb.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981263/anillo_3_piezas_morado_s4wake.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981245/anillo_2_piezas_lila_txzhxr.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981243/anillo_2_piezas_dorado_verde_byibag.webp"
    ],  
    "description": "Eleva tu estilo con este set de anillos de 2 piezas diseñado para quienes buscan versatilidad y personalidad en cada detalle. Su diseño unisex permite combinarlos libremente, ya sea usándolos juntos para un look más llamativo o por separado para un estilo minimalista.",
    "specs": [
      {
        "label": "Material",
        "value": "Aleacion Zinc/Hierro"
      }
    ],
    "rating": 4.7,
    "reviews": 31,
    "colors": [
      "#000000",
      "#EC1E1B",
      "#D0CCD1",
      "#000000",
      "#4D4D4D",
      "#916CEA",
      "#FFD700"
    ],
    "stock": 22,
    "tags": [],
    "fragrances": [],
    "isNew": true,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "accesorios-acero-quirurgico",
    "name": "Anillo, Pulsera Acero Quirurgico",
    "slug": "accesorios-acero-quirurgico",
    "category": "anillos",
    "price": 22.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "SABIDURÍA",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981283/pulsera_nudo_bruja_ace_rtjzdm.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981267/ani-pulsera_bruja_plateado_kw42cm.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981250/ani-pulsera_bruja_dorado_lb5pot.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981252/anillo_acero_bruja_dorado_ophtrc.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981279/anillo_acero_bruja_plateado_zomirp.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981279/anillo_acero_ala_plateado_rp77tm.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981283/anillo_acero_ala_dorado_vxrgnb.webp"
    ],
    "description": "Una colección diseñada para quienes buscan algo más que accesorios. Inspirada en símbolos místicos, esta línea combina protección, energía y estilo moderno, creando piezas que destacan por su significado y presencia.",
    "specs": [
      {
        "label": "Material",
        "value": "Acero Quirurgico"
      }
    ],
    "rating": 4.9,
    "reviews": 67,
    "colors": [
      "#D0CCD1",
      "#FFD700",
      "#FFD700",
      "#D0CCD1",
      "#D0CCD1",
      "#FFD700"
    ],
    "stock": 12,
    "tags": [],
    "fragrances": [],
    "isNew": false,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "anillos-conjunto",
    "name": "Anillo Serpientes Conjunto",
    "slug": "anillo-conjunto",
    "category": "anillos",
    "price": 29.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "VANGUARDIA",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981264/anillo_modelo_juego_llx0gm.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981282/anillo_juego_bronce_rfqq60.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981273/anillo_juego_negro_ltew3h.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981287/anillo_juego_plomo_euoo7e.webp"
    ],
    "description": "Atrévete a destacar con este exclusivo set de anillos de serpiente de 4 piezas, diseñado para transformar completamente el estilo de tu mano. Cada anillo se adapta a un dedo diferente creando una composición armoniosa, moderna y llena de actitud.",
    "specs": [
      {
        "label": "Material",
        "value": "Aleacion Zinc/Hierro"
      }
    ],
    "rating": 4.6,
    "reviews": 24,
    "colors": [
      "#FFFFA2",
      "#000000",
      "#4D4D4D"
    ],
    "stock": 14,
    "tags": [],
    "fragrances": [],
    "isNew": false,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "anillo-acero-antiEstres",
    "name": "Anillo Acero AntiEstres",
    "slug": "anillo-acero-antiEstres",
    "category": "anillos",
    "price": 24.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "FUERZA",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981276/anillo_stres_modelos_eijqu7.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981239/anillo_acero_plateado_eg2uz7.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981239/anillo_acero_plateado_cadena_azul_kfsl0e.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981238/anillo_acero_plateado_dorado_azmygy.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981238/anillo_acero_negro-plomo_ipz6rl.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981237/anillo_acero_negro_cadena_dorada_vavmxj.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981237/anillo_acero_dorado_xjj7ul.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981236/anillo_acero_arco_ires_krjit2.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981236/anillo_acero_azul_negro_chrfj1.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981233/anillo_acero_negro_qwnv8x.webp"
    ],
    "description": "Diseñado para hombres que buscan equilibrio y presencia, este anillo con cadena giratoria combina estilo moderno con una función relajante. Su mecanismo permite girar la cadena suavemente, ayudando a liberar tensión y mantener la concentración en cualquier momento del día",
    "specs": [
      {
        "label": "Material",
        "value": "Acero Inoxidable"
      }
    ],
    "rating": 4.8,
    "reviews": 42,
    "colors": [
      "#D0CCD1",
      "#003785",
      "#FFD700",
      "#000000",
      "#FFFFA2",
      "#FFD700",
      "#00FFF6",
      "#003785",
      "#000000"
    ],
    "stock": 10,
    "tags": [],
    "fragrances": [],
    "isNew": false,
    "isOnSale": false,
    "salePercent": null
  }, 
  {
    "id": "anillo-serpiente",
    "name": "Anillo Serpiente",
    "slug": "anillo-serpiente",
    "category": "anillos",
    "price": 14.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "RENOVACIÓN",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981288/anillo_modelo_B_tit6jz.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981275/anillo_serpiente_hierro_bronce_ddq294.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981244/anillo_serpiente_hierro_rosapalo_zpvs7e.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981244/anillo_serpiente_hierro_hjlsaz.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981255/anillo_serpiente_hierro_dorado_f2uipn.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981256/anillo_serpiente_hierro_plomo_t3qpfp.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981256/anillo_serpiente_hierroBronce_b3b1bc.webp",
    ],
    "description": "Inspirados en la simbología de la serpiente, representan transformación, poder y renovación, convirtiéndose en el accesorio perfecto para quienes buscan un look único y con carácter.",
    "specs": [
      {
        "label": "Material",
        "value": "Aleación de Hierro"
      }
    ],
    "rating": 4.9,
    "reviews": 89,
    "colors": [
      "#FFFFA2",
      "#CF4523FF",
      "#4D4D4D",
      "#FFD700",
      "#4D4D4D",
      "#FFFFA2"
    ],
    "stock": 30,
    "tags": [],
    "fragrances": [],
    "isNew": true,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "trepadores-anillo-garra",
    "name": "Trepadores y Anillos Garra",
    "slug": "trepadores-anillo-garra",
    "category": "anillos",
    "price": 19.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "ESTILO",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981270/trepadores_modelo_g0wthn.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981277/trepador_dorado_arete_s6vdcp.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981259/trepadores_plateado_oklzgx.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981252/anillo_modelo_dedo_oelowy.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981248/anillo_dedo_dorado_x2daxn.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981247/anillo_dedo_bronce_udghrx.webp"
    ],
    "description": "Representa libertad, evolución y fuerza interior. Un detalle sofisticado que eleva tu estilo.",
    "specs": [
      {
        "label": "Material",
        "value": "Aleación de Hierro"
      }
    ],
    "rating": 4.7,
    "reviews": 15,
    "colors": [
      "#FFD700",
      "#D0CCD1",
      "#D0CCD1",
      "#FFD700",
      "#FFFFA2"
    ],
    "stock": 15,
    "tags": [],
    "fragrances": [],
    "isNew": false,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "anillo-goticos",
    "name": "Anillos Goticos Aleacion Hierro",
    "slug": "anillo-gotico",
    "category": "anillos",
    "price": 15.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "MASIZO",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981282/anillo_modeloA_forfrp.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981237/anillo_mascara_jcvkph.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981241/anillo_rino_ya5njp.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981241/anillo_anubis_delgado_hxrzxh.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981241/anillo_anubis_grueso_v09jzg.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981242/anillo_sapo_rojo_hierro_b57nme.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981234/anillo_sapo_hierro_izyvco.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981240/anillo_pavo_aleacion_qdmu3a.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981245/anillo_calavera_hk7al3.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981246/anillo_toro_jtngl6.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981253/anillo_pavo_grande_aleacion_mnhyzj.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981253/anillo_aliens_j5cqgc.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981257/anillo_cabeza_aguila_zgcsau.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981260/anillo_cocodrilo_lbquce.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981261/anillo_dragon_aire_plomo_we5iso.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981263/anillo_elefante_bgtuq9.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981264/anillo_lobo_n13oq3.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981265/anillo_aguila_alas_votpt4.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981267/anillo_pavo_hierro_urznjn.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981268/anillo_buldog_bd50ku.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981271/anillo_dragon_aire_plomo_pja7af.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981272/anillo_elefante_hindu_cshvau.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981275/anillo_tigre_uni9zi.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981277/anillo_cobra_stk3aa.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981257/anillo_serpiente_mushu_aleacion_ajmbyf.webp"
    ],
    "description": "Fabricado en aleación resistente, combina durabilidad con un acabado elegante, ideal para quienes buscan destacar con un accesorio único y lleno de carácter.",
    "specs": [
      {
        "label": "Material",
        "value": "Aleación de Hierro/Zinc"
      }
    ],
    "rating": 4.8,
    "reviews": 12,
    "colors": [
      "#D0CCD1"
    ],
    "stock": 8,
    "tags": [],
    "fragrances": [],
    "isNew": false,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "anillos-aleacion-zinc",
    "name": "Anillos variados de Aleación Zinc",
    "slug": "anillos-aleacion-zinc",
    "category": "anillos",
    "price": 11.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "ZEN",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981289/anillo_modelos_tnrdfv.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981281/zorro_piedra_blanca_hb977y.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981278/anillo_dragon_agua_azul_mcme72.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981272/anillo_mariposa_bronce_pxs9th.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981271/zorro_piedra_rojo_dzn0cd.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981269/anillo_serpiente_zinc_flolrf.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981269/anillo_serpiente_plomo_aleacion_khtegz.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981268/anillo_serpiente_aleacion_negro_el4oj6.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981262/anillo_dragon_aleacion_qb8pvh.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981261/anillo_dragon_agua_rojo_m5ymsi.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981261/anillo_dragon_agua_verde_wfhqvj.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981260/zorro_piedra_morada_nvlibn.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981257/anillo_serpiente_zinc_aleacion_qtm1hc.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981255/anillo_serpiente_aleacion_plomo_nlx6mo.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981254/anillo_sapo_abrazo_aleacion_aigoit.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981250/anillo_garra_1_aleacion_vodeep.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981250/anillo_garra_2_aleacion_t2ldjo.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981249/anillo_dragon_cola_aleacion_gtqkak.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981235/anillo_garra_3_aleacion_wjeyub.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981234/anillo_sapo_aleacion_xoklwn.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981242/anillo_serpiente_3_zinc_negro_iue8hm.webp"
      
    ],
    "description": "Alineación energética vital. Siente el equilibrio de tus centros energéticos con este diseño que incorpora los 7 colores sagrados para traerte paz y enfoque.",
    "specs": [
      {
        "label": "Material",
        "value": "Aleación de Zinc"
      }
    ],
    "rating": 4.9,
    "reviews": 124,
    "colors": [
      "#C0C0C0"
    ],
    "stock": 40,
    "tags": [],
    "fragrances": [],
    "isNew": true,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "anillos-aleacion-HZ",
    "name": "Anillos goticos HZ",
    "slug": "anillos-aleacion-hz",
    "category": "anillos",
    "price": 22.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "PROTECCIÓN",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981288/anillo_modelo_B_tit6jz.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981254/anillo_promesa_negro_jldls7.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981263/anillo_gotico_craneo_wddvdt.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981267/anillo_promesa_plateado_nv3wa8.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981274/anillo_promesa_piedras_plateado_a6fad0.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774981280/ring_cabeza_drag_r0rbjz.webp"
    ],
    "description": "Escudo contra la negatividad. El antiguo símbolo celta del nudo de bruja te brinda protección y empoderamiento en cada paso de tu camino.",
    "specs": [
      {
        "label": "Material",
        "value": "Aleacion de HZ"
      }
    ],
    "rating": 4.8,
    "reviews": 93,
    "colors": [
      "#C0C0C0"
    ],
    "stock": 50,
    "tags": [],
    "fragrances": [],
    "isNew": false,
    "isOnSale": false,
    "salePercent": null
  },
  //********************************************************************//
  //           ESENCIAS - INCIENSOS
  //********************************************************************//
  {
    "id": "esencias-florales",
    "name": "Esencias Premium Florales",
    "slug": "esencias-florales",
    "category": "esencias",
    "price": 5.85,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "AROMATERAPIA",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774224611/goshopping/esencias_cono/esencias_florales.webp"
    ],
    "description": "Un viaje sensorial a través de los jardines más frescos. Nuestras esencias florales están diseñadas para armonizar tu entorno y revitalizar tus sentidos con la pureza de la naturaleza.",
    "specs": [],
    "rating": 4.9,
    "reviews": 142,
    "colors": [],
    "stock": 200,
    "tags": [],
    "fragrances": [
      { "name": "Amapola", "description": "Despierta la fuerza interior, tranquilidad y paz." },
      { "name": "Azucena", "description": "Paz, calma el dolor por rupturas y favorece la madurez." },
      { "name": "Bouquet", "description": "El aroma de la felicidad y la imaginación." },
      { "name": "Clavel", "description": "Bienestar general en casa y trabajo; da energía y salud." },
      { "name": "Flor de Azahar", "description": "Protector, purifica el ambiente y cura el susto." },
      { "name": "Gardenia", "description": "Protege, calma y restablece la paz en la pareja." },
      { "name": "Girasol", "description": "Restaurador del equilibrio, la salud y la alegría de vivir." },
      { "name": "Jazmín", "description": "Poderoso para la autoestima, optimismo y protección del hogar." },
      { "name": "Lavanda", "description": "Refresca y limpia el hogar de malas energías; ayuda a dormir." },
      { "name": "Lilas", "description": "Aporta serenidad, apacigua nervios y propicia buenas relaciones." },
      { "name": "Loto", "description": "Superación, libera la ansiedad y ayuda a superar crisis." },
      { "name": "Magnolia", "description": "Amor y belleza; ayuda a ver soluciones ante problemas." },
      { "name": "Margarita", "description": "Ayuda a aplacar las furias y los desplantes." },
      { "name": "Narciso Negro", "description": "Estimula la fidelidad, la entrega y la oración." },
      { "name": "Nardo", "description": "Ayuda a proteger la salud y brinda paz espiritual." },
      { "name": "Nenúfares", "description": "Da paz, es antidepresivo y desintoxica el espíritu." },
      { "name": "Orquídea", "description": "Ayuda a aceptar de manera natural el paso de los años." },
      { "name": "Ramillete de Novia", "description": "Pureza virginal, dulzura y confianza." },
      { "name": "Rosa", "description": "Amor, afecto y romanticismo; elimina la envidia y el odio." },
      { "name": "Violeta", "description": "Alivia el cansancio, quiebra encantos y espanta fantasmas." },
      { "name": "Ylang Ylang", "description": "Relajante, antidepresivo, afrodisíaco y energizante." }
    ],
    "isNew": false,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "esencias-frutales",
    "name": "Esencias Premium Frutales",
    "slug": "esencias-frutales",
    "category": "esencias",
    "price": 5.75,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "ENERGIZANTE",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774224613/goshopping/esencias_cono/esencias_frutales.webp"
    ],
    "description": "Aromas dulces y cítricos para el éxito, la alegría y la vitalidad. Llena tu ambiente de energía positiva y frescura incomparable.",
    "specs": [],
    "rating": 4.8,
    "reviews": 98,
    "colors": [],
    "stock": 200,
    "tags": [],
    "fragrances": [
      { "name": "Albaricoque", "description": "Alivia el estrés, genera paz y favorece buenas relaciones." },
      { "name": "Cereza", "description": "Juventud, alegría y felicidad; elimina trabas." },
      { "name": "Chirimoya", "description": "Fruta exquisita que limpia el aura y quita el decaimiento." },
      { "name": "Coco", "description": "Aroma placentero que relaja y calma los nervios." },
      { "name": "Durazno", "description": "Ayuda a controlar el insomnio y los celos." },
      { "name": "Frambuesa", "description": "Genera armonía, alegría y limpieza etérea." },
      { "name": "Fresa", "description": "Genera armonía, alegría y limpieza; estimula el \"niño interior\"." },
      { "name": "Limón", "description": "Limpia la melancolía, purifica y ayuda a la memoria." },
      { "name": "Lúcuma", "description": "Aroma de fertilidad y abundancia para el éxito familiar." },
      { "name": "Mandarina", "description": "Alegría, creatividad y prosperidad." },
      { "name": "Mango", "description": "Aroma romántico, sensual y placentero." },
      { "name": "Manzana", "description": "Salud, produce sueños agradables y libera miedos." },
      { "name": "Maracuyá", "description": "Tranquilizante y refrescante; excelente para el descanso." },
      { "name": "Melón", "description": "Brinda tranquilidad a personas de mal carácter." },
      { "name": "Naranja", "description": "Purificador y antidepresivo; proyecta alegría y verdad." },
      { "name": "Orange Mango", "description": "Aroma sensual que brinda placer y energía." },
      { "name": "Plátano", "description": "Aroma sensual, reduce la ansiedad y ayuda a la memoria." },
      { "name": "Uva", "description": "Da alegría y placer; se relaciona con el aprendizaje de la vida." }
    ],
    "isNew": false,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "esencias-maderosas",
    "name": "Esencias Premium Maderosas",
    "slug": "esencias-maderosas",
    "category": "esencias",
    "price": 5.68,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "ZEN",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774224614/goshopping/esencias_cono/esencias_maderosas.webp"
    ],
    "description": "Conéctate con la tierra y la profundidad del bosque. Toques de cedro y sándalo que proporcionan una base sólida y serena para tus momentos de meditación y paz.",
    "specs": [],
    "rating": 4.7,
    "reviews": 86,
    "colors": [],
    "stock": 200,
    "tags": [],
    "fragrances": [
      { "name": "Acacia", "description": "Árbol sagrado que ayuda a la inmortalidad del alma." },
      { "name": "Ámbar", "description": "Despierta la fuerza interior y la tranquilidad." },
      { "name": "Benjuí", "description": "Incrementa el poder personal y contrarresta influencias negativas." },
      { "name": "Cedro", "description": "Sedante, reduce la ansiedad y limpia el ambiente de negatividad." },
      { "name": "Mirra", "description": "Aroma para ofrendas; protege, bendice y santifica el hogar." },
      { "name": "Nag Champa", "description": "Aroma mágico para la meditación, oración y relajación." },
      { "name": "Palo Santo", "description": "Aroma de paz y espiritualidad; purifica el hogar." },
      { "name": "Pino", "description": "Aroma purificante, preserva y limpia malas energías del ambiente." },
      { "name": "Sándalo", "description": "El aroma más sagrado; da paz, perdón y perseverancia." },
      { "name": "Siete Raíces", "description": "Aroma afrodisíaco que despierta el erotismo." }
    ],
    "isNew": false,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "esencias-semillas-especias",
    "name": "Esencias Semillas y Especias",
    "slug": "esencias-semillas-especias",
    "category": "esencias",
    "price": 5.96,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "MÍSTICO",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774224617/goshopping/esencias_cono/esencias_semillas_y_especies.webp"
    ],
    "description": "Plantas y especias para salud, dinero y claridad mental. Refresca tu entorno y promueve el éxito cotidiano con aromas extraídos del alma de la naturaleza.",
    "specs": [],
    "rating": 4.6,
    "reviews": 54,
    "colors": [],
    "stock": 200,
    "tags": [],
    "fragrances": [
      { "name": "Albahaca", "description": "Aroma para quienes sienten oposición entre sexo y espíritu." },
      { "name": "Anís", "description": "Vacía la mente de pensamientos negativos; da sabiduría." },
      { "name": "Café", "description": "Estimula y vivifica los sentidos; ayuda a decidir entre dos caminos." },
      { "name": "Canela", "description": "Antidepresivo, armonioso y atrae el dinero." },
      { "name": "Cardamomo", "description": "Estimulante, da energía y es afrodisíaco." },
      { "name": "Citronela", "description": "Estimulante para el trabajo; elimina trabas al progreso." },
      { "name": "Clavo", "description": "Reduce agotamiento mental y ansiedad de malos hábitos (fumar/beber)." },
      { "name": "Coca", "description": "Poseedora de poderes místicos; relaja y abre la psiquis." },
      { "name": "Eucalipto", "description": "Protege a los niños de personas negativas; descongestionante." },
      { "name": "Hierba Buena", "description": "Aclara ideas, calma nervios y anula energías negativas." },
      { "name": "Hierba Luisa", "description": "Antiestrés, relajante y contra la fatiga mental." },
      { "name": "Hinojo", "description": "Equilibra estados de ánimo; ideal para estudio." },
      { "name": "Jengibre", "description": "Combate el agotamiento sexual y físico." },
      { "name": "Laurel", "description": "Armoniza el sistema nervioso y ayuda al equilibrio emocional." },
      { "name": "Menta", "description": "Estimulante mental, aclara ideas y reconforta el cuerpo." },
      { "name": "Romero", "description": "Energizante, curativo y promueve la longevidad." },
      { "name": "Ruda", "description": "Absorbe energías negativas y elimina dudas sobre uno mismo." },
      { "name": "Salvia", "description": "Relajante y motivador; ayuda en malestares femeninos." },
      { "name": "Tabaco", "description": "Aroma estimulante con efectos mágicos y religiosos." },
      { "name": "Té Verde", "description": "Relajante, libera del estrés y la tensión." },
      { "name": "Tomillo", "description": "Brinda apoyo y ayuda a ser constantes." },
      { "name": "Tulasi", "description": "Planta mística que otorga infinita misericordia." },
      { "name": "Verbena", "description": "Planta considerada mágica; aleja negatividad y malos espíritus." },
      { "name": "Vetiver", "description": "Aroma de estabilidad y seguridad; protege el dinero." }
    ],
    "isNew": false,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "esencias-misticas-esotericas",
    "name": "Esencias Misticas Esotericas",
    "slug": "esencias-misticas-esotericas",
    "category": "esencias",
    "price": 6.50,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "PASIÓN",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774224615/goshopping/esencias_cono/esencias_misticas_esotericas.webp"
    ],
    "description": "Nombres especiales enfocados en rituales, deidades y deseos específicos. Conecta con el misticismo y atrae energías superiores para tu beneficio y protección.",
    "specs": [],
    "rating": 4.8,
    "reviews": 45,
    "colors": [],
    "stock": 200,
    "tags": [],
    "fragrances": [
      { "name": "Abre Caminos", "description": "Limpia obstáculos para alcanzar objetivos." },
      { "name": "Abundancia", "description": "Ayuda a sentir que nuestra vida es abundante." },
      { "name": "Ángelus", "description": "Ideal para invocar a nuestros ángeles protectores." },
      { "name": "Buda", "description": "Estimula la espiritualidad y atrae prosperidad." },
      { "name": "Champagne", "description": "Lujoso para esplendor, festejo y celebración." },
      { "name": "Chandan", "description": "Mezcla de sándalo y jazmín; purificador y espiritual." },
      { "name": "Darshan", "description": "Aroma de beatitud y recogimiento." },
      { "name": "Diosa de las Huaringas", "description": "Limpieza y purificación profunda." },
      { "name": "Heliotropo", "description": "Purifica rituales y desvanece energías negativas." },
      { "name": "Kamasutra / Kananga", "description": "Para experiencias placenteras y sensuales." },
      { "name": "Llama Cliente", "description": "Incrementa la clientela en los negocios." },
      { "name": "Llama Dólar / Plata / Oro", "description": "Para atraer prosperidad y éxito en negocios." },
      { "name": "Miel de Amor", "description": "Atrae sentimientos de amor y reconforta." },
      { "name": "Mystic / Om", "description": "Equilibra el espíritu y los centros energéticos (chakras)." },
      { "name": "Night Queen", "description": "Genera simpatía y atracción del sexo opuesto." },
      { "name": "Opium", "description": "Místico, usado como estimulante psíquico." },
      { "name": "Pachouly", "description": "Atrae suerte en negocios, trabajo y dinero." },
      { "name": "Puzanga", "description": "Sintoniza y armoniza la fuerza sexual." },
      { "name": "Samsara / Shiva / Sai Flora", "description": "Para sabiduría, meditación y protección hindú." },
      { "name": "Siete Machos", "description": "Genera atracción con el sexo opuesto y da carácter." },
      { "name": "Atrayente / Amarrador", "description": "Para potenciar la atracción y sellar el amor." },
      { "name": "Bien Querer / Cariño", "description": "Potencia el verdadero amor y la energía afectiva." },
      { "name": "Brandy", "description": "Aroma de festejo, goce y Pleasure." },
      { "name": "Brisa Marina", "description": "Transporta al alma hacia la libertad y el amor." },
      { "name": "Desespero", "description": "Desespera y domina a la pareja." },
      { "name": "Dominio", "description": "Ayuda a ejercer poder sobre la pareja o negocios." },
      { "name": "Dulce Hogar / Dulces Sueños", "description": "Aleja males del hogar y ayuda al sueño." },
      { "name": "Favorecedora", "description": "Favorece la riqueza, el amor y la salud." },
      { "name": "Lluvia de Oro", "description": "Atrae éxito constante en los negocios." },
      { "name": "Madre Selva", "description": "Aroma curativo, genera amor y generosidad." },
      { "name": "Millonario", "description": "Atrae fortuna y éxito." },
      { "name": "Musk", "description": "Saca las mejores cualidades de nuestro ser y atrae físicamente." },
      { "name": "Piri Piri", "description": "Maravilloso regulador de la fertilidad." },
      { "name": "Prosperidad", "description": "Armoniza la fuerza sexual, próspero y exitoso." },
      { "name": "Quita Sales", "description": "Neutraliza males y previene enfermedades o daños." },
      { "name": "Relax / Antiestrés", "description": "Para la paz, el relax y la alegría." },
      { "name": "Retiro", "description": "Retira enemigos, envidias y maldad." },
      { "name": "Salud", "description": "Aroma místico que ayuda al control mental y bienestar." },
      { "name": "Sígueme Sígueme", "description": "Libera de obstáculos el camino hacia el ser amado." },
      { "name": "Ven Ven", "description": "Atrae al ser amado creando un vínculo indisoluble." }
    ],
    "isNew": false,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "incienso-abrecaminos",
    "name": "Incienso Abrecaminos",
    "slug": "incienso-abrecaminos",
    "category": "esencias",
    "price": 5.75,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "NUEVA ENERGÍA",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1/goshopping/esencias_cono/incienso_cono_abrecaminos.webp"
    ],
    "description": "Elimina obstáculos y atrae nuevas oportunidades. Este incienso está formulado para despejar tu camino hacia el éxito y la prosperidad.",
    "specs": [
      {
        "label": "Cantidad",
        "value": "10 unidades"
      },
      {
        "label": "Formato",
        "value": "Cono Cascada"
      }
    ],
    "rating": 5,
    "reviews": 31,
    "colors": [],
    "stock": 60,
    "tags": [],
    "fragrances": [],
    "isNew": true,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "incienso-antiestres",
    "name": "Incienso Antiestrés",
    "slug": "incienso-antiestres",
    "category": "esencias",
    "price": 5.95,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "RELAX",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1/goshopping/esencias_cono/incienso_cono_antiestres.webp"
    ],
    "description": "Tu refugio de paz tras un día agitado. Notas sedantes que calman el sistema nervioso y restauran el equilibrio emocional en tu hogar.",
    "specs": [
      {
        "label": "Cantidad",
        "value": "10 unidades"
      },
      {
        "label": "Efecto",
        "value": "Calmante Profundo"
      }
    ],
    "rating": 4.9,
    "reviews": 112,
    "colors": [],
    "stock": 100,
    "tags": [],
    "fragrances": [],
    "isNew": false,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "incienso-buddha",
    "name": "Incienso Buddha Zen",
    "slug": "incienso-buddha",
    "category": "esencias",
    "price": 5.65,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "MEDITACIÓN",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774224621/goshopping/esencias_cono/incienso_cono_buddha.webp"
    ],
    "description": "Inspiración divina para tu práctica espiritual. La fragancia del Buddha zen crea una atmósfera de respeto y contemplación profunda.",
    "specs": [
      {
        "label": "Cantidad",
        "value": "10 unidades"
      },
      {
        "label": "Uso",
        "value": "Meditación/Yoga"
      }
    ],
    "rating": 4.8,
    "reviews": 45,
    "colors": [],
    "stock": 40,
    "tags": [],
    "fragrances": [],
    "isNew": false,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "incienso-canabis",
    "name": "Incienso Cannabis Spirit",
    "slug": "incienso-canabis",
    "category": "esencias",
    "price": 5.75,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "HERBAL",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774224623/goshopping/esencias_cono/incienso_cono_canabis.webp"
    ],
    "description": "Relajación absoluta con un toque herbal místico. Una esencia alternativa para quienes buscan una desconexión total y una vibración diferente.",
    "specs": [
      {
        "label": "Cantidad",
        "value": "10 unidades"
      },
      {
        "label": "Aroma",
        "value": "Herbal/Místico"
      }
    ],
    "rating": 4.6,
    "reviews": 29,
    "colors": [],
    "stock": 25,
    "tags": [],
    "fragrances": [],
    "isNew": true,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "incienso-canela-manzana",
    "name": "Incienso Canela y Manzana",
    "slug": "incienso-canela-manzana",
    "category": "esencias",
    "price": 5.93,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "DULZURA",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774224624/goshopping/esencias_cono/incienso_cono_canela_manzana.webp"
    ],
    "description": "El calor de un hogar acogedor. La combinación clásica de canela y manzana que atrae la armonía familiar y la abundancia dulce.",
    "specs": [
      {
        "label": "Cantidad",
        "value": "10 unidades"
      },
      {
        "label": "Aroma",
        "value": "Gourmand/Cálido"
      }
    ],
    "rating": 4.8,
    "reviews": 76,
    "colors": [],
    "stock": 55,
    "tags": [],
    "fragrances": [],
    "isNew": false,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "incienso-canela-naranja",
    "name": "Incienso Canela y Naranja",
    "slug": "incienso-canela-naranja",
    "category": "esencias",
    "price": 5.89,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "VITALIDAD",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774224625/goshopping/esencias_cono/incienso_cono_canela_naranja.webp"
    ],
    "description": "Energía cítrica con un toque especiado. Ideal para revitalizar espacios de trabajo y atraer la creatividad con un aroma estimulante.",
    "specs": [
      {
        "label": "Cantidad",
        "value": "10 unidades"
      },
      {
        "label": "Aroma",
        "value": "Cítrico Especiado"
      }
    ],
    "rating": 4.7,
    "reviews": 34,
    "colors": [],
    "stock": 45,
    "tags": [],
    "fragrances": [],
    "isNew": true,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "incienso-canela",
    "name": "Incienso Canela Dulce",
    "slug": "incienso-canela",
    "category": "esencias",
    "price": 5.92,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "PROSPERIDAD",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774224626/goshopping/esencias_cono/incienso_cono_canela.webp"
    ],
    "description": "Atrayente natural de la buena fortuna. La canela pura es conocida por su poder para magnetizar el éxito y proteger la energía personal.",
    "specs": [
      {
        "label": "Cantidad",
        "value": "10 unidades"
      },
      {
        "label": "Uso",
        "value": "Ritual Abundancia"
      }
    ],
    "rating": 4.9,
    "reviews": 88,
    "colors": [],
    "stock": 70,
    "tags": [],
    "fragrances": [],
    "isNew": false,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "incienso-coco",
    "name": "Incienso Tropical Coco",
    "slug": "incienso-coco",
    "category": "esencias",
    "price": 5.39,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "EXÓTICO",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774224627/goshopping/esencias_cono/incienso_cono_coco.webp"
    ],
    "description": "Transporta tu mente a un paraíso tropical. El aroma dulce del coco purifica el ambiente y eleva el ánimo con notas suaves y veraniegas.",
    "specs": [
      {
        "label": "Cantidad",
        "value": "10 unidades"
      },
      {
        "label": "Aroma",
        "value": "Dulce Tropical"
      }
    ],
    "rating": 4.5,
    "reviews": 62,
    "colors": [],
    "stock": 40,
    "tags": [],
    "fragrances": [],
    "isNew": false,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "incienso-contra-envidia",
    "name": "Incienso Contra Envidia",
    "slug": "incienso-contra-envidia",
    "category": "esencias",
    "price": 5.96,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "ESCUDO",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774224628/goshopping/esencias_cono/incienso_cono_contra_envidia.webp"
    ],
    "description": "Tu barrera protectora contra energías negativas. Diseñado para neutralizar malos deseos y mantener la paz en tu entorno sagrado.",
    "specs": [
      {
        "label": "Cantidad",
        "value": "10 unidades"
      },
      {
        "label": "Nivel",
        "value": "Protección Alta"
      }
    ],
    "rating": 4.9,
    "reviews": 41,
    "colors": [],
    "stock": 30,
    "tags": [],
    "fragrances": [],
    "isNew": true,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "incienso-contra-mal",
    "name": "Incienso Contra Mal Vibras",
    "slug": "incienso-contra-mal",
    "category": "esencias",
    "price": 5.88,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "LIMPIEZA",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774224630/goshopping/esencias_cono/incienso_cono_contra_mal.webp"
    ],
    "description": "Exorciza la negatividad de cualquier espacio. Un incienso de limpieza profunda que corta bloqueos y restaura la luz en tu hogar.",
    "specs": [
      {
        "label": "Cantidad",
        "value": "10 unidades"
      },
      {
        "label": "Efecto",
        "value": "Descarga Energética"
      }
    ],
    "rating": 4.8,
    "reviews": 56,
    "colors": [],
    "stock": 25,
    "tags": [],
    "fragrances": [],
    "isNew": false,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "incienso-dulce-hogar",
    "name": "Incienso Dulce Hogar",
    "slug": "incienso-dulce-hogar",
    "category": "esencias",
    "price": 5.65,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "FAMILIAR",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774224631/goshopping/esencias_cono/incienso_cono_dulce_hogar.webp"
    ],
    "description": "Consolidación del amor y la unión familiar. Crea una atmósfera de ternura y comprensión colectiva en el corazón de tu casa.",
    "specs": [
      {
        "label": "Cantidad",
        "value": "10 unidades"
      },
      {
        "label": "Beneficio",
        "value": "Armonía Familiar"
      }
    ],
    "rating": 4.7,
    "reviews": 84,
    "colors": [],
    "stock": 50,
    "tags": [],
    "fragrances": [],
    "isNew": false,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "incienso-eucalipto",
    "name": "Incienso Eucalipto y Limón",
    "slug": "incienso-eucalipto",
    "category": "esencias",
    "price": 5.78,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "FRESCURA",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774224632/goshopping/esencias_cono/incienso_cono_eucalipto_limon.webp"
    ],
    "description": "Purificación del aire y revitalización respiratoria. La mezcla perfecta para despejar la mente y el ambiente con un aroma fresco y medicinal.",
    "specs": [
      {
        "label": "Cantidad",
        "value": "10 unidades"
      },
      {
        "label": "Aroma",
        "value": "Fresco/Cítrico"
      }
    ],
    "rating": 4.9,
    "reviews": 67,
    "colors": [],
    "stock": 48,
    "tags": [],
    "fragrances": [],
    "isNew": true,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "incienso-fresa",
    "name": "Incienso Fresa Silvestre",
    "slug": "incienso-fresa",
    "category": "esencias",
    "price": 5.76,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "ALEGRÍA",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774224634/goshopping/esencias_cono/incienso_cono_fresa.webp"
    ],
    "description": "Dulce tentación para el espíritu. La fresa silvestre atrae el amor propio y la positividad con sus notas frutales y encantadoras.",
    "specs": [
      {
        "label": "Cantidad",
        "value": "10 unidades"
      },
      {
        "label": "Aroma",
        "value": "Frutal Dulce"
      }
    ],
    "rating": 4.6,
    "reviews": 45,
    "colors": [],
    "stock": 40,
    "tags": [],
    "fragrances": [],
    "isNew": false,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "incienso-jasmin",
    "name": "Incienso Jazmín Místico",
    "slug": "incienso-jasmin",
    "category": "esencias",
    "price": 5.8,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "SADUCCIÓN",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774224635/goshopping/esencias_cono/incienso_cono_jazmin.webp"
    ],
    "description": "La flor de la noche en tu habitación. El jazmín atrae el amor espiritual y la belleza, creando un aura de elegancia y sofisticación.",
    "specs": [
      {
        "label": "Cantidad",
        "value": "10 unidades"
      },
      {
        "label": "Efecto",
        "value": "Atrayente de Amor"
      }
    ],
    "rating": 4.9,
    "reviews": 92,
    "colors": [],
    "stock": 35,
    "tags": [],
    "fragrances": [],
    "isNew": false,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "incienso-lavanda",
    "name": "Incienso Lavanda Dreams",
    "slug": "incienso-lavanda",
    "category": "esencias",
    "price": 5.85,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "SUEÑOS",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774224636/goshopping/esencias_cono/incienso_cono_lavanda.webp"
    ],
    "description": "El descanso definitivo bajo las estrellas. La lavanda es la reina de la relajación, induciendo un sueño reparador y una profunda paz interior.",
    "specs": [
      {
        "label": "Cantidad",
        "value": "10 unidades"
      },
      {
        "label": "Uso",
        "value": "Dormitorio/Relax"
      }
    ],
    "rating": 5,
    "reviews": 156,
    "colors": [],
    "stock": 120,
    "tags": [],
    "fragrances": [],
    "isNew": false,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "incienso-limpia-casa",
    "name": "Incienso Limpia Casa",
    "slug": "incienso-limpia-casa",
    "category": "esencias",
    "price": 5.95,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "RENOVACIÓN",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774224638/goshopping/esencias_cono/incienso_cono_limpia_casa.webp"
    ],
    "description": "Renueva el aire de cada habitación. Un incienso diario para mantener el flujo de energía positiva y la sensación de limpieza total en tu hogar.",
    "specs": [
      {
        "label": "Cantidad",
        "value": "10 unidades"
      },
      {
        "label": "Uso",
        "value": "Diario"
      }
    ],
    "rating": 4.7,
    "reviews": 38,
    "colors": [],
    "stock": 50,
    "tags": [],
    "fragrances": [],
    "isNew": false,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "incienso-limpieza-pesada",
    "name": "Incienso Limpieza Pesada",
    "slug": "incienso-limpieza-pesada",
    "category": "esencias",
    "price": 5.8,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "PROFUNDO",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774224639/goshopping/esencias_cono/incienso_cono_limpieza_pesada_y_descarga.webp"
    ],
    "description": "Eliminación radical de cargas energéticas negativas. La descarga definitiva para espacios que se sienten pesados o cargados tras eventos intensos.",
    "specs": [
      {
        "label": "Cantidad",
        "value": "10 unidades"
      },
      {
        "label": "Tipo",
        "value": "Limpieza Espiritual"
      }
    ],
    "rating": 5,
    "reviews": 24,
    "colors": [],
    "stock": 15,
    "tags": [],
    "fragrances": [],
    "isNew": true,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "incienso-llama-cliente",
    "name": "Incienso Atrae Clientes",
    "slug": "incienso-llama-cliente",
    "category": "esencias",
    "price": 5.7,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "NEGOCIOS",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774224640/goshopping/esencias_cono/incienso_cono_llama_cliente.webp"
    ],
    "description": "Imán para tu emprendimiento. Potencia las ventas y la afluencia de clientes con este incienso diseñado para el éxito comercial y la prosperidad.",
    "specs": [
      {
        "label": "Cantidad",
        "value": "10 unidades"
      },
      {
        "label": "Uso",
        "value": "Locales/Negocios"
      }
    ],
    "rating": 4.9,
    "reviews": 52,
    "colors": [],
    "stock": 40,
    "tags": [],
    "fragrances": [],
    "isNew": true,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "incienso-llama-plata",
    "name": "Incienso Llama Plata",
    "slug": "incienso-llama-plata",
    "category": "esencias",
    "price": 5.85,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "DINERO",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774224642/goshopping/esencias_cono/incienso_cono_llama_plata.webp"
    ],
    "description": "Conexión con el flujo constante del dinero. Atrae ganancias inesperadas y estabilidad financiera con la vibración argenta de este incienso sagrado.",
    "specs": [
      {
        "label": "Cantidad",
        "value": "10 unidades"
      },
      {
        "label": "Frecuencia",
        "value": "Atrayente Monetario"
      }
    ],
    "rating": 4.8,
    "reviews": 46,
    "colors": [],
    "stock": 35,
    "tags": [],
    "fragrances": [],
    "isNew": false,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "incienso-lluvia-oro",
    "name": "Incienso Lluvia de Oro",
    "slug": "incienso-lluvia-oro",
    "category": "esencias",
    "price": 5.95,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "RIQUEZA",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774224643/goshopping/esencias_cono/incienso_cono_lluvia_de_oro.webp"
    ],
    "description": "Cascada de bendiciones económicas. El oro simboliza el éxito total; este incienso te envuelve en una vibración de opulencia y abundancia infinita.",
    "specs": [
      {
        "label": "Cantidad",
        "value": "10 unidades"
      },
      {
        "label": "Símbolo",
        "value": "Éxito Absoluto"
      }
    ],
    "rating": 5,
    "reviews": 89,
    "colors": [],
    "stock": 45,
    "tags": [],
    "fragrances": [],
    "isNew": true,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "incienso-lluvia-plata",
    "name": "Incienso Lluvia de Plata",
    "slug": "incienso-lluvia-plata",
    "category": "esencias",
    "price": 5.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "SUERTE",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774224645/goshopping/esencias_cono/incienso_cono_lluvia_de_plata.webp"
    ],
    "description": "Protege tu patrimonio y atrae la suerte. La lluvia de plata mantiene tus finanzas en movimiento y protege tus ahorros de pérdidas energéticas.",
    "specs": [
      {
        "label": "Cantidad",
        "value": "10 unidades"
      },
      {
        "label": "Aroma",
        "value": "Fino/Elegante"
      }
    ],
    "rating": 4.7,
    "reviews": 33,
    "colors": [],
    "stock": 40,
    "tags": [],
    "fragrances": [],
    "isNew": false,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "incienso-manzana-verde",
    "name": "Incienso Manzana Verde",
    "slug": "incienso-manzana-verde",
    "category": "esencias",
    "price": 5.45,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "SALUD",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774224646/goshopping/esencias_cono/incienso_cono_manzana_verde.webp"
    ],
    "description": "Vitalidad renovada y salud vibrante. El aroma fresco de la manzana verde despeja el cansancio mental y atrae el bienestar físico a tu vida.",
    "specs": [
      {
        "label": "Cantidad",
        "value": "10 unidades"
      },
      {
        "label": "Aroma",
        "value": "Frutal Fresco"
      }
    ],
    "rating": 4.8,
    "reviews": 49,
    "colors": [],
    "stock": 42,
    "tags": [],
    "fragrances": [],
    "isNew": false,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "incienso-miel-rosa",
    "name": "Incienso Miel y Rosa",
    "slug": "incienso-miel-rosa",
    "category": "esencias",
    "price": 5.85,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "AMOR DULCE",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774224647/goshopping/esencias_cono/incienso_cono_miel_rosa.webp"
    ],
    "description": "Endulza tus relaciones y atrae el cariño. La miel y la rosa se funden para crear un clima de suavidad afectiva y atracción romántica.",
    "specs": [
      {
        "label": "Cantidad",
        "value": "10 unidades"
      },
      {
        "label": "Aroma",
        "value": "Floral Meloso"
      }
    ],
    "rating": 4.9,
    "reviews": 63,
    "colors": [],
    "stock": 30,
    "tags": [],
    "fragrances": [],
    "isNew": true,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "incienso-orquidea",
    "name": "Incienso Orquídea Exótica",
    "slug": "incienso-orquidea",
    "category": "esencias",
    "price": 5.75,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "LUJO",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774224649/goshopping/esencias_cono/incienso_cono_orquidea.webp"
    ],
    "description": "Distinción y elegancia en cada ráfaga de humo. La orquídea es símbolo de belleza suprema; este incienso eleva el estatus vibratorio de tus espacios.",
    "specs": [
      {
        "label": "Cantidad",
        "value": "10 unidades"
      },
      {
        "label": "Aroma",
        "value": "Floral Exótico"
      }
    ],
    "rating": 4.8,
    "reviews": 27,
    "colors": [],
    "stock": 20,
    "tags": [],
    "fragrances": [],
    "isNew": false,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "incienso-pachouli",
    "name": "Incienso Pachouli Earth",
    "slug": "incienso-pachouli",
    "category": "esencias",
    "price": 5.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "PASIÓN",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774224650/goshopping/esencias_cono/incienso_cono_pachouli.webp"
    ],
    "description": "Magnetismo terrenal y espiritual. El pachuli despierta los sentidos y protege el aura, siendo un aliado poderoso para la meditación y la sensualidad.",
    "specs": [
      {
        "label": "Cantidad",
        "value": "10 unidades"
      },
      {
        "label": "Aroma",
        "value": "Terroso/Especiado"
      }
    ],
    "rating": 4.9,
    "reviews": 71,
    "colors": [],
    "stock": 35,
    "tags": [],
    "fragrances": [],
    "isNew": false,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "incienso-palo-santo",
    "name": "Incienso Palo Santo Sagrado",
    "slug": "incienso-palo-santo",
    "category": "esencias",
    "price": 5.75,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "PURIFICACIÓN",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774224651/goshopping/esencias_cono/incienso_cono_palo_santo.webp"
    ],
    "description": "El misticismo de los Andes en tu hogar. Purifica, sana y reconecta con tu ser interior a través del aroma sagrado de la madera milenaria.",
    "specs": [
      {
        "label": "Cantidad",
        "value": "10 unidades"
      },
      {
        "label": "Origen",
        "value": "Natural/Sagrado"
      }
    ],
    "rating": 5,
    "reviews": 142,
    "colors": [],
    "stock": 65,
    "tags": [],
    "fragrances": [],
    "isNew": false,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "incienso-ponche-frutas",
    "name": "Incienso Ponche de Frutas",
    "slug": "incienso-ponche-frutas",
    "category": "esencias",
    "price": 5.55,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "CELEBRACIÓN",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774224652/goshopping/esencias_cono/incienso_cono_ponche_de_frutas.webp"
    ],
    "description": "Alegría compartida en cada rincón. Un aroma festivo que invita a la celebración y el buen humor en reuniones con seres queridos.",
    "specs": [
      {
        "label": "Cantidad",
        "value": "10 unidades"
      },
      {
        "label": "Aroma",
        "value": "Mix Frutal"
      }
    ],
    "rating": 4.6,
    "reviews": 31,
    "colors": [],
    "stock": 40,
    "tags": [],
    "fragrances": [],
    "isNew": true,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "incienso-rosa",
    "name": "Incienso Rosa Real",
    "slug": "incienso-rosa",
    "category": "esencias",
    "price": 5.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "AMOR ROYAL",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774224653/goshopping/esencias_cono/incienso_cono_rosa.webp"
    ],
    "description": "La esencia pura del romance. Abre tu corazón y atrae la ternura con el aroma floral más clásico y poderoso de la historia de la aromaterapia.",
    "specs": [
      {
        "label": "Cantidad",
        "value": "10 unidades"
      },
      {
        "label": "Aroma",
        "value": "Floral Clásico"
      }
    ],
    "rating": 4.9,
    "reviews": 85,
    "colors": [],
    "stock": 60,
    "tags": [],
    "fragrances": [],
    "isNew": false,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "incienso-sandalo",
    "name": "Incienso Sándalo Hindú",
    "slug": "incienso-sandalo",
    "category": "esencias",
    "price": 5.89,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "ESPIRITUAL",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774224656/goshopping/esencias_cono/incienso_cono_sandalo.webp"
    ],
    "description": "Serenidad milenaria para el alma. El sándalo es fundamental para la paz mental, el enfoque espiritual y la armonización de centros energéticos.",
    "specs": [
      {
        "label": "Cantidad",
        "value": "10 unidades"
      },
      {
        "label": "Aroma",
        "value": "Amaderado/Suave"
      }
    ],
    "rating": 5,
    "reviews": 128,
    "colors": [],
    "stock": 85,
    "tags": [],
    "fragrances": [],
    "isNew": false,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "incienso-siete-poderes",
    "name": "Incienso Siete Poderes",
    "slug": "incienso-siete-poderes",
    "category": "esencias",
    "price": 5.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "SUPREMO",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774224658/goshopping/esencias_cono/incienso_cono_siete_poderes.webp"
    ],
    "description": "La fuerza absoluta de la espiritualidad. Siete vibraciones combinadas para brindarte protección, salud, amor, dinero, paz, sabiduría y éxito.",
    "specs": [
      {
        "label": "Cantidad",
        "value": "10 unidades"
      },
      {
        "label": "Nivel",
        "value": "Poder Máximo"
      }
    ],
    "rating": 5,
    "reviews": 54,
    "colors": [],
    "stock": 15,
    "tags": [],
    "fragrances": [],
    "isNew": true,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "incienso-violeta-citronela",
    "name": "Incienso Violeta y Citronela",
    "slug": "incienso-violeta-citronela",
    "category": "esencias",
    "price": 5.89,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "PROTECCIÓN",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774224659/goshopping/esencias_cono/incienso_cono_violeta_y_citronela.webp"
    ],
    "description": "Transmutación energética con escudo natural. La violeta transmuta lo negativo en positivo mientras la citronela protege tu espacio de influencias externas.",
    "specs": [
      {
        "label": "Cantidad",
        "value": "10 unidades"
      },
      {
        "label": "Doble Efecto",
        "value": "Transmutar/Proteger"
      }
    ],
    "rating": 4.7,
    "reviews": 29,
    "colors": [],
    "stock": 35,
    "tags": [],
    "fragrances": [],
    "isNew": false,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "incienso-violeta",
    "name": "Incienso Violeta Suave",
    "slug": "incienso-violeta",
    "category": "esencias",
    "price": 5.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "TRANSMUTACIÓN",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774224660/goshopping/esencias_cono/incienso_cono_violeta.webp"
    ],
    "description": "Cambia tu energía de raíz. La violeta es la llama de la transmutación, ideal para convertir el dolor en sabiduría y la tristeza en esperanza.",
    "specs": [
      {
        "label": "Cantidad",
        "value": "10 unidades"
      },
      {
        "label": "Aroma",
        "value": "Floral Suave"
      }
    ],
    "rating": 4.8,
    "reviews": 51,
    "colors": [],
    "stock": 42,
    "tags": [],
    "fragrances": [],
    "isNew": false,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "incienso-salvia-blanca",
    "name": "Incienso Salvia Blanca",
    "slug": "incienso-salvia-blanca",
    "category": "esencias",
    "price": 5.89,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "-25%",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774224655/goshopping/esencias_cono/incienso_cono_salvia_blanca.webp"
    ],
    "description": "Purificación y energía positiva real. Siente la reconexión espiritual con el aroma sagrado del Palo Santo, recolectado de forma sostenible para elevar tu vibración.",
    "specs": [
      {
        "label": "Cantidad",
        "value": "10 unidades"
      },
      {
        "label": "Origen",
        "value": "Sudamérica"
      }
    ],
    "rating": 4.5,
    "reviews": 234,
    "colors": [],
    "stock": 45,
    "tags": [],
    "fragrances": [],
    "isNew": false,
    "isOnSale": true,
    "salePercent": 25
  },
 //********************************************************************//
 //           HIDROGEL
 //********************************************************************//
  {
    "id": "hidrogel-scar",
    "name": "Pistola SCAR ",
    "slug": "hidrogel-scar",
    "category": "hidrogel",
    "price": 169.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "MÁS VENDIDO",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774224668/goshopping/hidrogel/hidrogel_scar.webp"
    ],
    "description": "Domina el campo de batalla con la SCAR de hidrogel. Diseño ergonómico, disparo automático de alta velocidad y una precisión que te hará imparable en cada partida.",
    "specs": [
      {
        "label": "Municiones",
        "value": "5,000 incluidas"
      },
      {
        "label": "Batería",
        "value": "7.4V Recargable"
      },
      {
        "label": "Alcance",
        "value": "18-20 metros"
      }
    ],
    "rating": 4.8,
    "reviews": 312,
    "colors": [
      "-----"
    ],
    "stock": 25,
    "tags": [],
    "fragrances": [],
    "isNew": false,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "hidrogel-m416",
    "name": "Rifle M416 Pro Series",
    "slug": "hidrogel-m416",
    "category": "hidrogel",
    "price": 89.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "NUEVO",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774224667/goshopping/hidrogel/hidrogel_pistola_m416_rifle.webp"
    ],
    "description": "La joya de la corona en rifles de hidrogel. El M416 Pro ofrece una experiencia táctica realista con mira ajustable y culata expandible para máxima comodidad.",
    "specs": [
      {
        "label": "Modo",
        "value": "Automático / Semi"
      },
      {
        "label": "Capacidad",
        "value": "250 bolitas"
      },
      {
        "label": "Material",
        "value": "ABS Reforzado"
      }
    ],
    "rating": 4.9,
    "reviews": 89,
    "colors": [
      "----"
    ],
    "stock": 15,
    "tags": [],
    "fragrances": [],
    "isNew": true,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "hidrogel-vector",
    "name": "Subfusil Vector Vanguard",
    "slug": "hidrogel-vector",
    "category": "hidrogel",
    "price": 129.9,
    "oldPrice": 159.9,
    "currency": "PEN",
    "badge": "-19%",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774224670/goshopping/hidrogel/hidrogel_vector.webp"
    ],
    "description": "Velocidad de ráfaga inigualable. El diseño compacto del Vector lo hace ideal para enfrentamientos cercanos, proporcionando una agilidad táctica superior.",
    "specs": [
      {
        "label": "Cadencia",
        "value": "12 disparos/seg"
      },
      {
        "label": "Peso",
        "value": "1.2 kg"
      }
    ],
    "rating": 4.7,
    "reviews": 156,
    "colors": [
      "--------"
    ],
    "stock": 20,
    "tags": [],
    "fragrances": [],
    "isNew": false,
    "isOnSale": true,
    "salePercent": 19
  },
  {
    "id": "hidrogel-mp7a",
    "name": "Subfusil MP7A Elite",
    "slug": "hidrogel-mp7a",
    "category": "hidrogel",
    "price": 79.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "TACTICAL",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774224666/goshopping/hidrogel/hidrogel_mp7A.webp"
    ],
    "description": "Precisión alemana en tus manos. El MP7A combina el tamaño de una pistola con la potencia de un subfusil, perfecto para operaciones de asalto rápidas.",
    "specs": [
      {
        "label": "Diseño",
        "value": "PDW Modular"
      },
      {
        "label": "Alcance",
        "value": "18 metros"
      }
    ],
    "rating": 4.9,
    "reviews": 42,
    "colors": [
      "-------"
    ],
    "stock": 12,
    "tags": [],
    "fragrances": [],
    "isNew": true,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "hidrogel-mp7",
    "name": "Subfusil MP7 Compact",
    "slug": "hidrogel-mp7",
    "category": "hidrogel",
    "price": 79.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "ESENCIAL",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774224665/goshopping/hidrogel/hidrogel_mp7.webp"
    ],
    "description": "Ligereza y potencia. La versión compacta del MP7 es la entrada perfecta al mundo del hidrogel, ofreciendo fiabilidad y facilidad de uso inmejorables.",
    "specs": [
      {
        "label": "Peso",
        "value": "800g"
      }
    ],
    "rating": 4.6,
    "reviews": 67,
    "colors": [
      "--------"
    ],
    "stock": 30,
    "tags": [],
    "fragrances": [],
    "isNew": false,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "hidrogel-mp5k",
    "name": "Subfusil MP5K Stealth",
    "slug": "hidrogel-mp5k",
    "category": "hidrogel",
    "price": 135.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "POPULAR",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774224663/goshopping/hidrogel/hidrogel_mp5k.webp"
    ],
    "description": "Un clásico táctico. El MP5K Stealth es preferido por veteranos por su equilibrio perfecto entre ráfaga y control, envuelto en un acabado negro mate profesional.",
    "specs": [
      {
        "label": "Bateria",
        "value": "5.7v"
      }
    ],
    "rating": 4.8,
    "reviews": 124,
    "colors": [
      "----"
    ],
    "stock": 18,
    "tags": [],
    "fragrances": [],
    "isNew": false,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "hidrogel-mp5ka",
    "name": "Subfusil MP5Ka Tactical",
    "slug": "hidrogel-mp5ka",
    "category": "hidrogel",
    "price": 95.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "AGIL",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774224664/goshopping/hidrogel/hidrogel_mp5Ka.webp"
    ],
    "description": "Personalización al extremo. El MP5Ka ofrece rieles tácticos para añadir tus propios accesorios, adaptándose a cualquier estilo de juego en el campo.",
    "specs": [
      {
        "label": "Funcion",
        "value": "Raga - Tiro por uno"
      }
    ],
    "rating": 4.5,
    "reviews": 54,
    "colors": [
      "--------"
    ],
    "stock": 22,
    "tags": [],
    "fragrances": [],
    "isNew": true,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "hidrogel-battle-robot",
    "name": "Robot de Batalla Hidrogel",
    "slug": "hidrogel-battle-robot",
    "category": "hidrogel",
    "price": 149.9,
    "oldPrice": 199.9,
    "currency": "PEN",
    "badge": "EXCLUSIVO",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774224662/goshopping/hidrogel/hidrogel_battle_robot.webp"
    ],
    "description": "El futuro de la diversión. Este tanque robótico lanza ráfagas de hidrogel mientras realiza maniobras de 360°, controlado remotamente para un dominio absoluto del área.",
    "specs": [
      {
        "label": "Movimiento",
        "value": "360° Omnidireccional"
      },
      {
        "label": "Potencia",
        "value": "Alcance 20 metros"
      }
    ],
    "rating": 5,
    "reviews": 28,
    "colors": [
      ""
    ],
    "stock": 8,
    "tags": [],
    "fragrances": [],
    "isNew": true,
    "isOnSale": true,
    "salePercent": 10
  },
  {
    "id": "hidrogel-water-gun",
    "name": "Pistola de Agua Hidro-Blast",
    "slug": "hidrogel-water-gun",
    "category": "hidrogel",
    "price": 39.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "VERANO",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774224671/goshopping/hidrogel/pistola_water_gun.webp"
    ],
    "description": "Diversión refrescante. Potente chorro de agua con diseño deportivo y carga rápida, ideal para las batallas mas intensas bajo el sol de verano.",
    "specs": [
      {
        "label": "Capacidad",
        "value": "600ml"
      }
    ],
    "rating": 4.4,
    "reviews": 142,
    "colors": [
      "-----"
    ],
    "stock": 50,
    "tags": [],
    "fragrances": [],
    "isNew": false,
    "isOnSale": false,
    "salePercent": null
  },

  //********************************************************************//
  //           DIFUSORES - HUMIFICADORES
  //********************************************************************//
  {
    "id": "difusor-cilindrico",
    "name": "Difusor Cilindrico",
    "slug": "difusor-cilindrico",
    "category": "humidificadores",
    "price": 39.90,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "NUEVO",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774323356/difusor_cilindros_modelos_gti4fv.png",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774323318/difusor_cilindrico_madera_oscura_nt3wdv.png",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774323301/difusor_cilindrico_madera_clara_k6iqod.png",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774323316/difusor_cilindrico_blanco_cwrovg.png",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774323311/difusor_cilindrico_cafe_mate_z445ch.png",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774323321/difusor_cilindrico_rosado_udovvq.png"
    ],
    "description": "Lleva la naturaleza a tu habitación. Experimenta el efecto relajante de la lluvia y la suavidad de una nube en 3D, diseñado para transformar tu descanso en una experiencia mágica.",
    "specs": [
      {
        "label": "Capacidad",
        "value": "400ml"
      },
      {
        "label": "Efecto",
        "value": "Difusor de Aromas - Filtro de Algodon"
      }
    ],
    "rating": 4.7,
    "reviews": 89,
    "colors": [
      "#430D06",
      "#AA7444",
      "#FFFFFF",
      "#2B0F01FF",
      "#FBB2A3"
    ],
    "stock": 15,
    "tags": [],
    "fragrances": [],
    "isNew": true,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "difusor-redondo-con-diseño",
    "name": "Difusor Diseño Redondo",
    "slug": "difusor-redondo-con-diseño",
    "category": "humidificadores",
    "price": 23.9,
    "oldPrice": 29.9,
    "currency": "PEN",
    "badge": "-20%",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774323353/difusor_redondo_con_dise%C3%B1os_mfb6jo.png",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774323264/difusor_redondo_dise%C3%B1o_rojo_d6qvgf.jpg",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774323261/difusor_redondo_dise%C3%B1o_azul_mj76uk.jpg"
    ],
    "description": "Equilibrio natural en tu espacio. El acabado en bambú auténtico se une a la aromaterapia de lujo para brindarte un ambiente zen y sofisticado en cualquier rincón.",
    "specs": [
      {
        "label": "Capacidad",
        "value": "120ml"
      },
      {
        "label": "Material",
        "value": "PVC + ABS, Filtro de Algodon"
      }
    ],
    "rating": 4.4,
    "reviews": 156,
    "colors": [
      "#EC1E1B",
      "#003785"
    ],
    "stock": 30,
    "tags": [],
    "fragrances": [],
    "isNew": false,
    "isOnSale": true,
    "salePercent": 20
  },
  {
    "id": "difusor-ovalado",
    "name": "Difusor Ovalado",
    "slug": "difusor-ovalado",
    "category": "humidificadores",
    "price": 29.9,
    "oldPrice": 44.9,
    "currency": "PEN",
    "badge": "PROMO",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774323342/difusor_ovales_dl1cci.png",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774323324/difusor_oval_madera_claro_vsdlxh.png",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774323258/difusor_oval_cafe_io3xur.png"
    ],
    "description": "Calidez hipnótica en tu hogar. Siente la fuerza del fuego con un efecto de llama ultra-realista que vaporiza tus esencias favoritas, creando un ambiente acogedor y vanguardista.",
    "specs": [
      {
        "label": "Capacidad",
        "value": "220ml"
      },
      {
        "label": "Luz",
        "value": "LED RGB, Filtro de Algodon"
      }
    ],
    "rating": 4.9,
    "reviews": 56,
    "colors": [
      "#AA7444",
      "#430D06"
    ],
    "stock": 18,
    "tags": [],
    "fragrances": [],
    "isNew": false,
    "isOnSale": true,
    "salePercent": 14
  },
  {
    "id": "difusor-corona",
    "name": "Difusor Corona Royal",
    "slug": "difusor-corona",
    "category": "humidificadores",
    "price": 53.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "ELEGANTE",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774322834/difusor_corona_uvkzvy.webp"
    ],
    "description": "La joya de tu bienestar. Un diseño majestuoso que corona tus espacios con pureza y elegancia, transformando cada respiro en un momento de distinción real.",
    "specs": [
      {
        "label": "Capacidad",
        "value": "200ml - Motor"
      }
    ],
    "rating": 4.6,
    "reviews": 22,
    "colors": [
      "#AA7444"
    ],
    "stock": 25,
    "tags": [],
    "fragrances": [],
    "isNew": false,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "difusor-mini-gota",
    "name": "Difusor Mini Gota",
    "slug": "difusor-mini-gota",
    "category": "humidificadores",
    "price": 29.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "PREMIUM",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774323338/difusor_mini_gotas_hjeoqs.png",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774323307/difusor_mini_gota_cafe_b0vu5p.png",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774323246/difusor_mini_gota_madera_claro_jq6540.png"
    ],
    "description": "Sonido y naturaleza en perfecta armonía. Este difusor de diseño alpino integra un parlante de alta fidelidad para envolver tus sentidos en una experiencia sensorial completa.",
    "specs": [
      {
        "label": "Funciones",
        "value": "Difusor de Aroma, Filtro de Algodon"
      },
      {
        "label": "Luz",
        "value": "Ambiente Cálido"
      }
    ],
    "rating": 5,
    "reviews": 12,
    "colors": [
      "#430D06",
      "#AA7444"
    ],
    "stock": 10,
    "tags": [],
    "fragrances": [],
    "isNew": true,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "difusor-multicolor-parlante",
    "name": "Difusor Multicolor Gota Reversible",
    "slug": "difusor-multicolor-parlante",
    "category": "humidificadores",
    "price": 79.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "FIESTA",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774323246/difusor_multicolor_con_parlante_hancrl.webp"
    ],
    "description": "Energía vibrante para tu hogar. Combina cromoterapia y tu música favorita con este difusor inteligente que cambia de color al ritmo de tus mejores canciones.",
    "specs": [
      {
        "label": "Conexión",
        "value": "Bluetooth"
      },
      {
        "label": "Colores",
        "value": "RGB Dinámico, Parlante, Motor"
      }
    ],
    "rating": 4.8,
    "reviews": 34,
    "colors": [
      "#FFFFFF"
    ],
    "stock": 15,
    "tags": [],
    "fragrances": [],
    "isNew": false,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "difusor-florero",
    "name": "Difusor Florero",
    "slug": "difusor-florero",
    "category": "humidificadores",
    "price": 32.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "ESTILO",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774323335/difusor_floreros_ewnbbp.png",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774323309/difusor_florero_cafe_eomess.png",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774323314/difusor_florero_claro_y5gci3.png"
    ],
    "description": "Arte circular que respira. Un diseño esférico con patrones intrincados que proyectan sombras mágicas mientras purifican el aire con elegancia mística.",
    "specs": [
      {
        "label": "Material",
        "value": "ABS de Lujo , Filtro de Algodon"
      },
      {
        "label": "Capacidad",
        "value": "150ml"
      }
    ],
    "rating": 4.7,
    "reviews": 41,
    "colors": [
      "#430D06",
      "#AA7444"
    ],
    "stock": 20,
    "tags": [],
    "fragrances": [],
    "isNew": true,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "humidificador-rgb",
    "name": "Humidificador RGB",
    "slug": "humidificador-rgb",
    "category": "humidificadores",
    "price": 39.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "ZEN",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774323331/humificador_rgb_e6hzbi.png"
    ],
    "description": "La fluidez de los aromas capturada en una pieza de diseño que aporta calma inmediata a cualquier rincón de tu hogar.",
    "specs": [
      {
        "label": "Capacidad",
        "value": "200ml"
      },
      {
        "label": "Ruido",
        "value": "Silencioso"
      }
    ],
    "rating": 4.9,
    "reviews": 67,
    "colors": [
      "#FFFFFF"
    ],
    "stock": 30,
    "tags": [],
    "fragrances": [],
    "isNew": false,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "difusor-montaña-parlante",
    "name": "Difusor Montaña Parlante",
    "slug": "difusor-montaña-parlante",
    "category": "humidificadores",
    "price": 79.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "PREMIUM",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774323276/disufor_parlante_principal_yoayxh.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774323329/difusor_parlante_volcan_negro_rd2r4x.png",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774323326/difusor_parlante_volcan_claro_mvoykl.png"
    ],
    "description": "Bienestar que te acompaña. Perfecto para tu escritorio o espacios reducidos, este mini difusor mantiene tu entorno fresco con un tamaño compacto y eficiente.",
    "specs": [
      {
        "label": "Conexión",
        "value": "USB"
      },
      {
        "label": "Luz",
        "value": "RGB, Filtro de Algodon"
      }
    ],
    "rating": 4.5,
    "reviews": 112,
    "colors": [
      "#000000",
      "#FFFFFF"
    ],
    "stock": 50,
    "tags": [],
    "fragrances": [],
    "isNew": true,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "difusor-luna-osito",
    "name": "Difusor Luna Osito Dream",
    "slug": "difusor-luna-osito",
    "category": "humidificadores",
    "price": 27.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "KIDS",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774323242/difusor_luna_osito_zcyq9f.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774323250/difusor_osito_luna_blanco_pricipal_yseawk.png",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774323252/difusor_osito_luna_rosa_secundaria_n4z7my.png",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774323255/difusor_osito_luna_verde_secundaria_sm7zgd.png"
    ],
    "description": "Dulces sueños garantizados. La luz suave de la luna y la compañía de un tierno osito crean el ambiente perfecto para el descanso de los más pequeños.",
    "specs": [
      {
        "label": "Luz",
        "value": "Quita miedos LED"
      },
      {
        "label": "Seguridad",
        "value": "Apagado Automático, Filtro de Algodon"
      }
    ],
    "rating": 4.9,
    "reviews": 88,
    "colors": [
      "#FFFFFF",
      "#FBB2A3",
      "#90EE90"
    ],
    "stock": 18,
    "tags": [],
    "fragrances": [],
    "isNew": true,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "difusor-flama-volcan",
    "name": "Difusor Flama Volcán",
    "slug": "difusor-flama-volcan",
    "category": "humidificadores",
    "price": 66.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "MODERNO",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774323215/difusor_flama_volcan_principal_xnxg1t.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774323304/difusor_flama_volcan_blanco_vldl41.png",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774323214/difusor_flama_volcan_negro_zqgvdj.png"
    ],
    "description": "Iluminación circular que inspira. Una llama de luz infinita rodea este difusor de alta tecnología, permitiéndote personalizar el ambiente con millones de colores.",
    "specs": [
      {
        "label": "Diseño",
        "value": "Luz LED"
      },
      {
        "label": "Modos",
        "value": "Flama , Motor"
      }
    ],
    "rating": 4.7,
    "reviews": 29,
    "colors": [
      "#FFFFFF",
      "#000000"
    ],
    "stock": 12,
    "tags": [],
    "fragrances": [],
    "isNew": false,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "difusor-jarra-zen",
    "name": "Difusor Jarra Zen",
    "slug": "difusor-jarra-zen",
    "category": "humidificadores",
    "price": 27.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "CHIC",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774323225/difusor_jarra_liso_principal_urhcej.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774323298/difusor_jarra_liso_craquelado_z6hyru.png",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774323288/difusor_jarra_liso_madera_claro_za1ahf.png",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774323259/difusor_jarra_liso_cafe_aoahku.png"
    ],
    "description": "Elegancia con su forma orgánica y suave retro lo convierten en el complemento ideal para una decoración moderna y sofisticada.",
    "specs": [
      {
        "label": "Capacidad",
        "value": "220ml"
      },
      {
        "label": "Liso",
        "value": "Color liso / Craquelado, Filtro de Algodon"
      }
    ],
    "rating": 4.6,
    "reviews": 24,
    "colors": [
      "#F0EBD7",
      "#AA7444",
      "#2B0F01"
    ],
    "stock": 25,
    "tags": [],
    "fragrances": [],
    "isNew": false,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "difusor-gota",
    "name": "Difusor Gota",
    "slug": "difusor-gota",
    "category": "humidificadores",
    "price": 49.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "TOP",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774323220/difusor_gota_principal_h7r9mb.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774323295/difusor_gota_color_cafe_obt4up.png",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774323294/difusor_gota_color_madera_m34r5p.png"
    ],
    "description": "Pureza y color en armonía. Un diseño maderoso que cobra vida con efectos de luz espectaculares para un ambiente siempre renovado.",
    "specs": [
      {
        "label": "Diseño",
        "value": "Maderoso"
      },
      {
        "label": "Luz",
        "value": "Multicolor, Motor"
      }
    ],
    "rating": 4.8,
    "reviews": 52,
    "colors": [
          "#430D06",
          "#AA7444"
        ],
    "stock": 35,
    "tags": [],
    "fragrances": [],
    "isNew": true,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "difusor-gota901",
    "name": "Difusor Gota 901",
    "slug": "difusor-gota901",
    "category": "humidificadores",
    "price": 62.99,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "PREMIUN",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774323286/difusor_gota901_mlhcxc.png",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774323291/difusor_gota901_cafe_lebqj3.png",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774323223/difusor_gota901_madera_clara_gx1k9l.png"
    ],
    "description": "Texturas que conectan. Inspirado en lo artesanal, este difusor combina tecnología ultrasónica con una estética rústica y acogedora.",
    "specs": [  
      {
        "label": "Estilo",
        "value": "Cesta Artística"
      }
    ],
    "rating": 4.9,
    "reviews": 16,
    "colors": [
      "#430D06",
      "#AA7444"
    ],
    "stock": 10,
    "tags": [],
    "fragrances": [],
    "isNew": true,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "difusor-jarra-oro",
    "name": "Difusor Jarra Espejo",
    "slug": "difusor-jarra-oro",
    "category": "humidificadores",
    "price": 28.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "LUJO",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774323227/difusor_jarra_oros_principal_mselsi.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774323278/humificador_magenta_jarra_130ml_idvazz.jpg",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774323281/humificador_oro_rosa_jarra_130ml_s37nrb.jpg",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774323283/humificador_turquesa_jarra_130ml_kce8rk.jpg"
    ],
    "description": "Suavidad de seda en tu espacio. Un diseño de jarrón clásico con acabado ultra-liso que encaja perfectamente en los ambientes más exclusivos.",
    "specs": [
      {
        "label": "Material",
        "value": "Pvc Espejo"
      },
      {
        "label": "Capacidad",
        "value": "130ml, Filtro de Algodon"
      }
    ],
    "rating": 4.8,
    "reviews": 21,
    "colors": [
      "#FF00FF",
      "#CF4523FF",
      "#40E0D0"
    ],
    "stock": 15,
    "tags": [],
    "fragrances": [],
    "isNew": false,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "difusor-redondo-basMade",
    "name": "Difusor Redondo BasMade",
    "slug": "difusor-redondo-basMade",
    "category": "humidificadores",
    "price": 49.9,
    "oldPrice": null,
    "currency": "PEN",  
    "badge": "LAMPARA",
    "badgeColor": "RGB",
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774323239/difusor_lampara_circular_rgb_jxehxs.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774323273/difusor_redondo_basMade_trck9r.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774323239/difusor_lampara_circular_rgb_jxehxs.webp"
    ],
    "description": "Lampara con difusor. El acabado con toques de madera, convirtiéndolo en una pieza decorativa única y funcional.",
    "specs": [
      {
        "label": "Acabado",
        "value": "Pvc, Motor"
      }
    ],
    "rating": 4.9,
    "reviews": 14,
    "colors": [
      "#AA7444",
      "#FFFFFF"
    ],
    "stock": 8,
    "tags": [],
    "fragrances": [],
    "isNew": true,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "difusor-lluvia",
    "name": "Difusor Lluvia",
    "slug": "difusor-lluvia",
    "category": "humidificadores",
    "price": 83.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "PRESTIGIO",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774323266/difusor_y_lluvia_rgb_ybeism.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774323266/difusor_y_lluvia_rgb_ybeism.webp"
    ],
    "description": "Diseño real para tu descanso. Una silueta elegante y equilibrada que aporta una nota de distinción clásica a tu ritual de bienestar diario.",
    "specs": [
      {
        "label": "Capacidad",
        "value": "400ml"
      },
      {
        "label": "Luz",
        "value": "RGB, Motor"
      }
    ],
    "rating": 4.7,
    "reviews": 19,
    "colors": [
      "#FFFFFF"
    ],
    "stock": 20,
    "tags": [],
    "fragrances": [],
    "isNew": false,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "difusor-y",
    "name": "Difusor Clasico Y",
    "slug": "difusor-y",
    "category": "humidificadores",
    "price": 16.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "CLASICO",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774323269/difusor_Y_eu9kge.webp"
    ],
    "description": "Puntos de serenidad. ofreciendo un flujo constante de aroma, muy versatil para llevarlo.",
    "specs": [
      {
        "label": "Estilo",
        "value": "Zen Moderno"
      }
    ],
    "rating": 4.6,
    "reviews": 31,
    "colors": [
      "#FFFFFF",
      "#FBB2A3",
      "#40E0D0"
    ],
    "stock": 22,
    "tags": [],
    "fragrances": [],
    "isNew": false,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "difusor-jarron-temporizador",
    "name": "Difusor Jarron temporizador",
    "slug": "difusor-jarron-temporizador",
    "category": "humidificadores",
    "price": 69.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "URBANO",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774323236/difusor_jarron_principal_juibcx.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774323234/difusor_jarron_madera_clara_fjjzl1.jpg",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774323232/difusor_jarron_cafe_jstjxz.jpg",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774323232/difusor_jarron_craquelado_hm6igq.jpg"
    ],
    "description": "Funcionalidad sin complicaciones. Un diseño tipo jarro con líneas limpias y acabado mate, ideal para quienes buscan eficiencia con un toque moderno.",
    "specs": [
      {
        "label": "Material",
        "value": "ABS Alta Densidad"
      },
      {
        "label": "Capacidad",
        "value": "500ml, Motor, 8h"
      }
    ],
    "rating": 4.5,
    "reviews": 45,
    "colors": [
      "#AA7444",
      "#430d06",
      "#F0EBD7"
    ],
    "stock": 25,
    "tags": [],
    "fragrances": [],
    "isNew": true,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "difusor-fuego",
    "name": "Difusor Fuego",
    "slug": "difusor-fuego",
    "category": "humidificadores",
    "price": 52.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "PURO",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774323218/difusor_fuego_v9ghe9.webp"
    ],
    "description": "LLamas de fuego. Una evolución del diseño clásico de Humificadores con efecto de llamas, prioriza la simplicidad y la potencia de su tecnología ultrasónica.",
    "specs": [
      {
        "label": "Diseño",
        "value": "Pure Line"
      }
    ],
    "rating": 4.6,
    "reviews": 27,
    "colors": [
      "#000000",
      "#FFFFFF"
    ],
    "stock": 30,
    "tags": [],
    "fragrances": [],
    "isNew": false,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "difusor-cesta-mt",
    "name": "Difusor Cesta",
    "slug": "difusor-cesta-mt",
    "category": "humidificadores",
    "price": 65.9,
    "oldPrice": 85.9,
    "currency": "PEN",
    "badge": "TOP",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774323210/difusor_cesta_mt_n0xg1k.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774323210/difusor_cesta_mt_n0xg1k.webp"
    ],
    "description": "Solidez y sofisticación. Esta versión premium de nuestro diseño tejido ofrece acabados unicos para tu fragancia favorita.",
    "specs": [
      {
        "label": "Capacidad",
        "value": "200ml"
      },
      {
        "label": "Material",
        "value": "Reforzado, motor"
      }
    ],
    "rating": 4.8,
    "reviews": 39,
    "colors": [
      "#AA7444"
    ],
    "stock": 15,
    "tags": [],
    "fragrances": [],
    "isNew": true,
    "isOnSale": false,
    "salePercent": null
  },
  //********************************************************************//
  //           ACCESORIOS MASCOTAS
  //********************************************************************//
  {
    "id": "casco-modelo",
    "name": "Casco Motero",
    "slug": "casco-modelo",
    "category": "mascotas",
    "price": 49.9,
    "oldPrice": 59.9,
    "currency": "PEN",
    "badge": "SEGURIDAD",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774755657/casco_modelo_nwamxa.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774755656/casco_dog_S_mf3ble.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774755655/casco_dog_M_lradyu.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774755655/casco_dog_L_zt1pi2.webp"
    ],
    "variants": [
    {
      "name": "Talla S",
      "price": 49.9,
      "oldPrice": 59.9
    },
    {
      "name": "Talla M",
      "price": 59.9,
      "oldPrice":null
    },
    {
      "name": "Talla L",
      "price": 69.9,
      "oldPrice": null
    }
  ],
    "description": "Seguridad máxima para tus aventuras extremas. Este casco táctico para mascotas está fabricado en polímero de alta resistencia, ideal para paseos en moto o actividades al aire libre que requieran protección adicional.",
    "specs": [
      {
        "label": "Material",
        "value": "Polímero ABS"
      },
      {
        "label": "Tallas",
        "value": "S / M / L"
      }
    ],
    "rating": 4.9,
    "reviews": 124,
    "colors": [
      "#000000",
      "#000000",
      "#000000"

    ],
    "stock": 15,
    "tags": [],
    "fragrances": [],
    "isNew": false,
    "isOnSale": true,
    "salePercent": 22
  },
  {
    "id": "lente-modelo-aviador",
    "name": "Lente Modelo Aviador",
    "slug": "lente-modelo-aviador",
    "category": "mascotas",
    "price": 15.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "ESTILO",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774755691/gafa_animal_aviador_sqwgfp.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774762465/gaja_aviador_verde_zsohhb.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774762460/gafa_aviador_rosa_cbfkly.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774762459/gafa_aviador_trasparente_yvjsmv.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774762459/gafa_aviador_azul_x01hnp.webp"
    ],
    "description": "El look clásico para el piloto más fiel. Gafas tipo aviador con protección UV400 y correa elástica ajustable para asegurar comodidad y firmeza durante la acción.",
    "specs": [
      {
        "label": "Protección",
        "value": "Vidrio"
      },
      {
        "label": "Montura",
        "value": "Metal"
      }
    ],
    "rating": 4.8,
    "reviews": 156,
    "colors": [
      "#90EE90",
      "#fbb2a3",
      "#f0f0f0",
      "#003785"
    ],
    "stock": 30,
    "tags": [],
    "fragrances": [],
    "isNew": true,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "lente-dog-redondo-modelo",
    "name": "Lente Dog Redondo",
    "slug": "lente-dog-redondo-modelo",
    "category": "mascotas",
    "price": 15.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "VINTAGE",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774755693/gafa_animal_redondo_va_ksnhzw.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774755690/gafa_redondo_ahumado_lsj9n7.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774755689/gafa_redonda_rosa_x9z5w2.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774755688/gafa_redonda_rojo_gpfwjr.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774755687/gafa_redonda_negro_gif13a.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774755686/gafa_redonda_lila_mftfpx.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774755684/gafa_redonda_cafe_dee5x6.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774755684/gafa_redonda_azul_tm6x3t.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774755682/gafa_redonda_amarilla_t3nzsy.webp"      
    ],
    "description": "Elegancia vintage para sesiones de fotos inolvidables. Estas gafas redondas aportan un toque intelectual y chic a tu mascota, perfectas para redes sociales.",
    "specs": [
      {
        "label": "Estilo",
        "value": "Retro Circular"
      }
    ],
    "rating": 4.7,
    "reviews": 89,
    "colors": [
      "#69503C",
      "#FBB2A3",
      "#EC1E1B",
      "#4D4D4D",
      "#916CEA",
      "#2B0F01",
      "#003785",
      "#FFFFA2"   
    ],
    "stock": 25,
    "tags": [],
    "fragrances": [],
    "isNew": false,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "lente-cat-corazon",
    "name": "Lente Cat Corazón Minimalista",
    "slug": "lente-cat-corazon",
    "category": "mascotas",
    "price": 15.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "TRENDY",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774755694/gafa_cat_corazon_paxhwx.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774762464/gafa_oval_trasparente_lb0g5e.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774762463/gafa_oval_minimalista_jnmf9g.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774762462/gafa_corazon_rojo_isgfmm.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774755701/lente_corazon_verdeamarela_roazhf.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774755700/lente_corazon_verde_oeujp1.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774755699/lente_corazon_rosa_iapulx.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774755697/lente_corazon_lila_gzjbsg.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774755695/lente_corazon_amarillo_pt2zwp.webp"
    ],
    "description": "Diseño moderno para la ciudad. Gafas con montura reforzada que combinan perfectamente con cualquier arnés o collar, ideales para el día a día urbano.",
    "specs": [
      {
        "label": "Montura",
        "value": "Reforzada"
      }
    ],
    "rating": 4.6,
    "reviews": 67,
    "colors": [
      "#F0F0F0",
      "#C99036",
      "#EC1E1B",
      "#03BB85",
      "#90EE90",
      "#FBB2A3",
      "#916CEA",
      "#FFFFA2"
    ],
    "stock": 20,
    "tags": [],
    "fragrances": [],
    "isNew": true,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "lente-cat-redonda-modelo",
    "name": "Lente Cat Espejo",
    "slug": "lente-cat-redonda-modelo",
    "category": "mascotas",
    "price": 15.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "LOVELY",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774755692/gafa_animal_redonda_uzippo.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774755678/gafa_espejo_verde_gqq6yk.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774755677/gafa_espejo_tornasol_s6pphy.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774755676/gafa_espejo_plateado_ng0ixp.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774755675/gafa_espejo_marino_rgaosd.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774755674/gafa_espejo_azul_xwjhah.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774755673/gafa_espejo_arco_ires_uh9glo.webp"
    ],
    "description": "Derrite corazones con este accesorio súper dulce. Gafas en forma de corazón para las mascotas más cariñosas de la casa, ligeras y muy cómodas.",
    "specs": [
      {
        "label": "Diseño",
        "value": "Heart Shaped"
      }
    ],
    "rating": 4.9,
    "reviews": 212,
    "colors": [
      " #90EE90",
      "#E7C500",
      "#D0CCD1",
      "#588100",
      "#003785",
      "#00FFF6"
    ],
    "stock": 35,
    "tags": [],
    "fragrances": [],
    "isNew": false,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "lente-rectangulo-mascotas",
    "name": "Lente Rectangulo",
    "slug": "lente-rectangulo-mascotas",
    "category": "mascotas",
    "price": 15.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "COOL",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774755703/lente_rectangulo_mascotas_uh8u7k.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774755672/gafa_cuadrada_verde_khxela.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774755671/gafa_cuadrada_trasparente_t7a2af.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774755670/gafa_cuadrada_rosa_vw6mug.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774755669/gafa_cuadrada_naraja_vh6vfx.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774755668/gafa_cuadrada_celeste_k6bo24.webp"
    ],
    "description": "Reflejos de pura onda. Lentes con acabado de espejo multicolor que protegen del sol intenso y ofrecen un look futurista y audaz para tu mascota.",
    "specs": [
      {
        "label": "Lente",
        "value": "Efecto Espejo"
      }
    ],
    "rating": 4.8,
    "reviews": 45,
    "colors": [
      "#90EE90",
      "#000000",
      "#FBB2A3",
      "#FF9800",
      "#94E7FF"
    ],
    "stock": 15,
    "tags": [],
    "fragrances": [],
    "isNew": false,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "lente-moto-dog-modelo",
    "name": "Lente Moto Dog Flex",
    "slug": "lente-moto-dog-modelo",
    "category": "mascotas",
    "price": 18.9,
    "oldPrice": 69.9,
    "currency": "PEN",
    "badge": "TOP",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774755703/lente_moto_dog_xcmf0c.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774755666/dog_rosado_simple_xfnw4i.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774755665/dog_negro_simple_xl3cwl.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774755657/dog_blanco_simple_e7cir2.webp"
    ],
    "description": "Resistencia total al viento y al polvo. El accesorio indispensable para mascotas que viajan en moto, con acolchado interno para evitar irritaciones.",
    "specs": [
      {
        "label": "Extra",
        "value": "Acolchado Foam"
      }
    ],
    "rating": 5,
    "reviews": 78, 
    "colors": [
      "#FBB2A3",
      "#000000",
      "#FFFFFF"

    ],
    "stock": 12,
    "tags": [],
    "fragrances": [],
    "isNew": true,
    "isOnSale": true,
    "salePercent": 20
  },
  {
    "id": "sombrero-cat",
    "name": "Sombrero Western Cowboy Pet",
    "slug": "sombrero-cat",
    "category": "mascotas",
    "price": 18.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "DIVERTIDO",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774755713/sombrero_cat_dcqrad.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774755709/sombrero_dog_rosa_m2lxgx.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774755707/sombrero_dog_negro_htuga2.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774755708/sombrero_dog_rojo_treha6.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774755705/sombrero_dog_beige_wk8nhv.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774755705/sombrero_dog_guinda_xbj2do.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774755711/sombrero_medidas_S_gvrucv.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774755706/sombrero_dog_marron_tvy4sg.webp"
    ],
    "description": "¡Llegó el sheriff a la ciudad! Sombrero estilo western con cordón ajustable bajo la barbilla, ideal para disfraces o simplemente para lucir diferente.",
    "specs": [
      {
        "label": "Estilo",
        "value": "Cowboy"
      },
      {
        "label": "Ajuste",
        "value": "Cordón con Freno"
      },
      {
        "label": "TALLA",
        "value": "TODAS SON TALLAS 'S'"
      }
    ],
    "rating": 4.5,
    "reviews": 94,
    "colors": [
      "#FBB2A3",
      "#000000",
      "#EC1E1B",
      "#DDDAAB",
      "#73063B",
      "#2B0F01",
      "#955F34"
    ],
    "stock": 40,
    "tags": [],
    "fragrances": [],
    "isNew": false,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "sombrero-dog",
    "name": "Sombrero Elegance Pet",
    "slug": "sombrero-dog",
    "category": "mascotas",
    "price": 19.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "GENTLEMAN",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774755714/sombrero_dog_zmj1gt.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774755714/sombrerp_dog_cafe_itgtfj.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774755707/sombrero_dog_negro_htuga2.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774755706/sombrero_dog_marron_tvy4sg.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774755710/sombrero_medidas_M_bk7ebf.webp"
    ],
    "description": "Para las ocasiones más especiales. Un bombín miniatura que aporta clase y distinción, fabricado en fieltro suave para que tu mascota se sienta cómoda.",
    "specs": [
      {
        "label": "Estilo",
        "value": "Cowboy"
      },
      {
        "label": "Ajuste",
        "value": "Cordón con Freno"
      },
      {
        "label": "TALLA",
        "value": "TODAS SON TALLAS 'M"
      }
    ],
    "rating": 4.8,
    "reviews": 36,
    "colors": [
      "#2B0F01",
      "#000000",
      "#955F34"

    ],
    "stock": 15,
    "tags": [],
    "fragrances": [],
    "isNew": true,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "sombrero-moto-dog",
    "name": "Lentes Explorador de Moto Dog",
    "slug": "sombrero-moto-dog",
    "category": "mascotas",
    "price": 24.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "EXPLORADOR",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774755696/gafa_moto_animal_i8appk.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774755681/gafa_moto_dog_traslucido_fx0mmm.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774755680/gafa_moto_dog_polarizado_ngk9yb.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774755679/gafa_moto_dog_espejo_v0rfoj.webp"
    ],
    "description": "Protección total contra el viento y el polvo. El accesorio indispensable para mascotas que viajan en moto, con acolchado interno para evitar irritaciones.",
    "specs": [
      {
        "label": "Uso",
        "value": "Protección Solar"
      },
      {
        "label": "Talla",
        "value": "Para Mascotas medias a grandes"
      }
    ],
    "rating": 4.6,
    "reviews": 52,
    "colors": [
      "#F0F0F0",
      "#69503C",
      "#D0CCD1"
    ],
    "stock": 18,
    "tags": [],
    "fragrances": [],
    "isNew": false,
    "isOnSale": false,
    "salePercent": null
  },
  //********************************************************************//
  //           NOVEDADES
  //********************************************************************//
  {
    "id": "nov-casco-dino",
    "name": "Casco Dino",
    "slug": "nov-casco-dino",
    "category": "novedades",
    "price": 49.9,
    "oldPrice": 59.9,
    "currency": "PEN",
    "badge": "NUEVO",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1/goshopping/novedades/casco_dino.webp"
    ],
    "description": "Descubre esta increíble pieza de nuestra colección de Novedades. Un artículo con detalles únicos que destaca en cualquier colección o ambiente.",
    "specs": [
      {
        "label": "Categoría",
        "value": "Novedad Exclusiva"
      }
    ],
    "rating": 4.8,
    "reviews": 25,
    "colors": [
      "#C0C0C0"
    ],
    "stock": 15,
    "tags": [],
    "fragrances": [],
    "isNew": true,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "nov-esfera-ada-madera-6",
    "name": "Esfera Ada Madera 6cm",
    "slug": "nov-esfera-ada-madera-6",
    "category": "novedades",
    "price": 19.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "DECOR",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1/goshopping/novedades/esfera_ada_madera_6.webp"
    ],
    "description": "Descubre esta increíble pieza de nuestra colección de Novedades. Un artículo con detalles únicos que destaca en cualquier colección o ambiente.",
    "specs": [
      {
        "label": "Categoría",
        "value": "Novedad Exclusiva- luz amarilla"
      }
    ],
    "rating": 4.8,
    "reviews": 25,
    "colors": [
      "#C0C0C0"
    ],
    "stock": 15,
    "tags": [],
    "fragrances": [],
    "isNew": true,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "esfera-astronauta-6rgb",
    "name": "Esfera Astronauta 6cm rgb",
    "slug": "esfera-astronauta-6rgb",
    "category": "novedades",
    "price": 29.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "DECOR",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1/goshopping/novedades/esfera_astronauta_6rgb.webp"
    ],
    "description": "Descubre esta increíble pieza de nuestra colección de Novedades. Un artículo con detalles únicos que destaca en cualquier colección o ambiente.",
    "specs": [
      {
        "label": "Categoría",
        "value": "Novedad Exclusiva - Luces rgb"
      }
    ],
    "rating": 4.8,
    "reviews": 25,
    "colors": [
      "#C0C0C0"
    ],
    "stock": 15,
    "tags": [],
    "fragrances": [],
    "isNew": true,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "nov-esfera-dragon-madera-8",
    "name": "Esfera Dragon Madera 8cm",
    "slug": "nov-esfera-dragon-madera-8",
    "category": "novedades",
    "price": 24.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "DECOR",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1/goshopping/novedades/esfera_dragon_madera_8.webp"
    ],
    "description": "Descubre esta increíble pieza de nuestra colección de Novedades. Un artículo con detalles únicos que destaca en cualquier colección o ambiente.",
    "specs": [
      {
        "label": "Categoría",
        "value": "Novedad Exclusiva"
      }
    ],
    "rating": 4.8,
    "reviews": 25,
    "colors": [
      "#C0C0C0"
    ],
    "stock": 15,
    "tags": [],
    "fragrances": [],
    "isNew": true,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "esfera-luna-madera-8",
    "name": "Esfera Luna Madera 8cm",
    "slug": "esfera-luna-madera-8",
    "category": "novedades",
    "price": 24.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "DECOR",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1/goshopping/novedades/esfera_luna_madera_8.webp"
    ],
    "description": "Descubre esta increíble pieza de nuestra colección de Novedades. Un artículo con detalles únicos que destaca en cualquier colección o ambiente.",
    "specs": [
      {
        "label": "Categoría",
        "value": "Novedad Exclusiva - Luz Amarilla"
      }
    ],
    "rating": 4.8,
    "reviews": 25,
    "colors": [
      "#C0C0C0"
    ],
    "stock": 15,
    "tags": [],
    "fragrances": [],
    "isNew": true,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "esfera-oso-madera-6",
    "name": "Esfera Oso Madera 6cm",
    "slug": "esfera-oso-madera-6",
    "category": "novedades",
    "price": 19.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "DECOR",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1/goshopping/novedades/esfera_oso_madera_6.webp"
    ],
    "description": "Descubre esta increíble pieza de nuestra colección de Novedades. Un artículo con detalles únicos que destaca en cualquier colección o ambiente.",
    "specs": [
      {
        "label": "Categoría",
        "value": "Novedad Exclusiva - Luz Amarilla"
      }
    ],
    "rating": 4.8,
    "reviews": 25,
    "colors": [
      "#C0C0C0"
    ],
    "stock": 15,
    "tags": [],
    "fragrances": [],
    "isNew": true,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "esfera-rgbb-astro-luna6",
    "name": "Esfera RgbB Astro Luna 6cm",
    "slug": "esfera-rgbb-astro-luna6",
    "category": "novedades",
    "price": 29.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "DECOR",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1/goshopping/novedades/esfera_rgbB_astro_luna6.jpeg"
    ],
    "description": "Descubre esta increíble pieza de nuestra colección de Novedades. Un artículo con detalles únicos que destaca en cualquier colección o ambiente.",
    "specs": [
      {
        "label": "Categoría",
        "value": "Novedad Exclusiva - Luz rgb"
      }
    ],
    "rating": 4.8,
    "reviews": 25,
    "colors": [
      "#C0C0C0"
    ],
    "stock": 15,
    "tags": [],
    "fragrances": [],
    "isNew": true,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "esfera-rgbb-astro-reno6",
    "name": "Esfera RgbB Astro Reno 6cm",
    "slug": "esfera-rgbb-astro-reno6",
    "category": "novedades",
    "price": 29.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "DECOR",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1/goshopping/novedades/esfera_rgbB_astro_reno6.webp"
    ],
    "description": "Descubre esta increíble pieza de nuestra colección de Novedades. Un artículo con detalles únicos que destaca en cualquier colección o ambiente.",
    "specs": [
      {
        "label": "Categoría",
        "value": "Novedad Exclusiva - Luz rgb"
      }
    ],
    "rating": 4.8,
    "reviews": 25,
    "colors": [
      "#C0C0C0"
    ],
    "stock": 15,
    "tags": [],
    "fragrances": [],
    "isNew": true,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "esfera-rgbb-astro-sistema6",
    "name": "Esfera RgbB Astro Sistema 6cm",
    "slug": "esfera-rgbb-astro-sistema6",
    "category": "novedades",
    "price": 29.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "DECOR",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1/goshopping/novedades/esfera_rgbB_astro_sistema6.webp"
    ],
    "description": "Descubre esta increíble pieza de nuestra colección de Novedades. Un artículo con detalles únicos que destaca en cualquier colección o ambiente.",
    "specs": [
      {
        "label": "Categoría",
        "value": "Novedad Exclusiva - Luz rgb"
      }
    ],
    "rating": 4.8,
    "reviews": 25,
    "colors": [
      "#C0C0C0"
    ],
    "stock": 15,
    "tags": [],
    "fragrances": [],
    "isNew": true,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "esfera-sistema-madera-8",
    "name": "Esfera Sistema Madera 8cm",
    "slug": "esfera-sistema-madera-8",
    "category": "novedades",
    "price": 24.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "DECOR",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1/goshopping/novedades/esfera_sistema_madera_8.webp"
    ],
    "description": "Descubre esta increíble pieza de nuestra colección de Novedades. Un artículo con detalles únicos que destaca en cualquier colección o ambiente.",
    "specs": [
      {
        "label": "Categoría",
        "value": "Novedad Exclusiva - Luz Amarillo"
      }
    ],
    "rating": 4.8,
    "reviews": 25,
    "colors": [
      "#C0C0C0"
    ],
    "stock": 15,
    "tags": [],
    "fragrances": [],
    "isNew": true,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "moneda-bitcoin",
    "name": "Moneda Bitcoin - Ethereum",
    "slug": "moneda-bitcoin",
    "category": "novedades",
    "price": 15.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "COLECCIÓN",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774224706/goshopping/novedades/moneda_lateral_bitcoin.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774224703/goshopping/novedades/moneda_botcoin.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774224703/goshopping/novedades/moneda_botcoin.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774224705/goshopping/novedades/moneda_ethereum.webp",
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774224705/goshopping/novedades/moneda_ethereum.webp"
    ],
    "description": "Descubre esta increíble pieza de nuestra colección de Novedades. Un artículo con detalles únicos que destaca en cualquier colección o ambiente.",
    "specs": [
      {
        "label": "Categoría",
        "value": "Novedad Exclusiva"
      }
    ],
    "rating": 4.8,
    "reviews": 25,
    "colors": [
      "#FFD700",
      "#D0CCD1",
      "#FFD700",
      "#D0CCD1"
    ],
    "stock": 15,
    "tags": [],
    "fragrances": [],
    "isNew": true,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "quemador-taza-pq",
    "name": "Taza Caballero Pq",
    "slug": "quemador-taza-pq",
    "category": "novedades",
    "price": 31.9,
    "oldPrice": 39.9,
    "currency": "PEN",
    "badge": "ÉPICO",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1/goshopping/novedades/quemador_taza_pq.webp"
    ],
    "description": "Descubre esta increíble pieza de nuestra colección de Novedades. Un artículo con detalles únicos que destaca en cualquier colección o ambiente.",
    "specs": [
      {
        "label": "Categoría",
        "value": "Novedad Exclusiva"
      }
    ],
    "rating": 4.8,
    "reviews": 25,
    "colors": [
      "#C0C0C0"
    ],
    "stock": 15,
    "tags": [],
    "fragrances": [],
    "isNew": true,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "taza-thor-pq",
    "name": "Taza Thor Pq",
    "slug": "taza-thor-pq",
    "category": "novedades",
    "price": 31.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "ÉPICO",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774224708/goshopping/novedades/taza_thor_pq.webp"
    ],
    "description": "Descubre esta increíble pieza de nuestra colección de Novedades. Un artículo con detalles únicos que destaca en cualquier colección o ambiente.",
    "specs": [
      {
        "label": "Categoría",
        "value": "Novedad Exclusiva"
      }
    ],
    "rating": 4.8,
    "reviews": 25,
    "colors": [
      "#C0C0C0"
    ],
    "stock": 15,
    "tags": [],
    "fragrances": [],
    "isNew": true,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "taza-barril-med",
    "name": "Taza Barril Med",
    "slug": "taza-barril-med",
    "category": "novedades",
    "price": 45.9,
    "oldPrice": 59.9,
    "currency": "PEN",
    "badge": "ÉPICO",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1/goshopping/novedades/taza_barril_med.webp"
    ],
    "description": "Descubre esta increíble pieza de nuestra colección de Novedades. Un artículo con detalles únicos que destaca en cualquier colección o ambiente.",
    "specs": [
      {
        "label": "Categoría",
        "value": "Novedad Exclusiva"
      }
    ],
    "rating": 4.8,
    "reviews": 25,
    "colors": [
      "#C0C0C0"
    ],
    "stock": 15,
    "tags": [],
    "fragrances": [],
    "isNew": true,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "taza-cruz-pq",
    "name": "Taza Cruz Pq",
    "slug": "taza-cruz-pq",
    "category": "novedades",
    "price": 31.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "ÉPICO",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1/goshopping/novedades/taza_cruz_pq.webp"
    ],
    "description": "Descubre esta increíble pieza de nuestra colección de Novedades. Un artículo con detalles únicos que destaca en cualquier colección o ambiente.",
    "specs": [
      {
        "label": "Categoría",
        "value": "Novedad Exclusiva"
      }
    ],
    "rating": 4.8,
    "reviews": 25,
    "colors": [
      "#C0C0C0"
    ],
    "stock": 15,
    "tags": [],
    "fragrances": [],
    "isNew": true,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "taza-dragarra-gr",
    "name": "Taza Dragarra Gr",
    "slug": "taza-dragarra-gr",
    "category": "novedades",
    "price": 49.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "ÉPICO",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1/goshopping/novedades/taza_dragarra_gr.webp"
    ],
    "description": "Descubre esta increíble pieza de nuestra colección de Novedades. Un artículo con detalles únicos que destaca en cualquier colección o ambiente.",
    "specs": [
      {
        "label": "Categoría",
        "value": "Novedad Exclusiva"
      }
    ],
    "rating": 4.8,
    "reviews": 25,
    "colors": [
      "#C0C0C0"
    ],
    "stock": 15,
    "tags": [],
    "fragrances": [],
    "isNew": true,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "taza-dragema-gr",
    "name": "Taza Dragema Gr",
    "slug": "taza-dragema-gr",
    "category": "novedades",
    "price": 49.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "ÉPICO",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1/goshopping/novedades/taza_dragema_gr.webp"
    ],
    "description": "Descubre esta increíble pieza de nuestra colección de Novedades. Un artículo con detalles únicos que destaca en cualquier colección o ambiente.",
    "specs": [
      {
        "label": "Categoría",
        "value": "Novedad Exclusiva"
      }
    ],
    "rating": 4.8,
    "reviews": 25,
    "colors": [
      "#C0C0C0"
    ],
    "stock": 15,
    "tags": [],
    "fragrances": [],
    "isNew": true,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "taza-dragon-col-gr",
    "name": "Taza Dragon Col Gr",
    "slug": "taza-dragon-col-gr",
    "category": "novedades",
    "price": 51.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "ÉPICO",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1/goshopping/novedades/taza_dragon_col_gr.webp"
    ],
    "description": "Descubre esta increíble pieza de nuestra colección de Novedades. Un artículo con detalles únicos que destaca en cualquier colección o ambiente.",
    "specs": [
      {
        "label": "Categoría",
        "value": "Novedad Exclusiva"
      }
    ],
    "rating": 4.8,
    "reviews": 25,
    "colors": [
      "#C0C0C0"
    ],
    "stock": 15,
    "tags": [],
    "fragrances": [],
    "isNew": true,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "taza-game-thrones-med",
    "name": "Taza Game Thrones Med",
    "slug": "taza-game-thrones-med",
    "category": "novedades",
    "price": 35.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "ÉPICO",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1/goshopping/novedades/taza_game_thrones_med.webp"
    ],
    "description": "Descubre esta increíble pieza de nuestra colección de Novedades. Un artículo con detalles únicos que destaca en cualquier colección o ambiente.",
    "specs": [
      {
        "label": "Categoría",
        "value": "Novedad Exclusiva"
      }
    ],
    "rating": 4.8,
    "reviews": 25,
    "colors": [
      "#C0C0C0"
    ],
    "stock": 15,
    "tags": [],
    "fragrances": [],
    "isNew": true,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "taza-hp-amar-gr",
    "name": "Taza HP Amarillo Gr",
    "slug": "taza-hp-amar-gr",
    "category": "novedades",
    "price": 61.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "ÉPICO",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1/goshopping/novedades/taza_HP_amar_gr.webp"
    ],
    "description": "Descubre esta increíble pieza de nuestra colección de Novedades. Un artículo con detalles únicos que destaca en cualquier colección o ambiente.",
    "specs": [
      {
        "label": "Categoría",
        "value": "Novedad Exclusiva"
      }
    ],
    "rating": 4.8,
    "reviews": 25,
    "colors": [
      "#C0C0C0"
    ],
    "stock": 15,
    "tags": [],
    "fragrances": [],
    "isNew": true,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "taza-hp-red-gr",
    "name": "Taza HP Red Gr",
    "slug": "taza-hp-red-gr",
    "category": "novedades",
    "price": 61.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "ÉPICO",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1/goshopping/novedades/taza_HP_red_gr.webp"
    ],
    "description": "Descubre esta increíble pieza de nuestra colección de Novedades. Un artículo con detalles únicos que destaca en cualquier colección o ambiente.",
    "specs": [
      {
        "label": "Categoría",
        "value": "Novedad Exclusiva"
      }
    ],
    "rating": 4.8,
    "reviews": 25,
    "colors": [
      "#C0C0C0"
    ],
    "stock": 15,
    "tags": [],
    "fragrances": [],
    "isNew": true,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "taza-medie-pq",
    "name": "Taza Medieval Pq",
    "slug": "taza-medie-pq",
    "category": "novedades",
    "price": 29.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "ÉPICO",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1/goshopping/novedades/taza_medie_pq.webp"
    ],
    "description": "Descubre esta increíble pieza de nuestra colección de Novedades. Un artículo con detalles únicos que destaca en cualquier colección o ambiente.",
    "specs": [
      {
        "label": "Categoría",
        "value": "Novedad Exclusiva"
      }
    ],
    "rating": 4.8,
    "reviews": 25,
    "colors": [
      "#C0C0C0"
    ],
    "stock": 15,
    "tags": [],
    "fragrances": [],
    "isNew": true,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "taza-vikingo-gr",
    "name": "Taza Vikingo Gr",
    "slug": "taza-vikingo-gr",
    "category": "novedades",
    "price": 51.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "ÉPICO",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1/goshopping/novedades/taza_vikingo_gr.webp"
    ],
    "description": "Descubre esta increíble pieza de nuestra colección de Novedades. Un artículo con detalles únicos que destaca en cualquier colección o ambiente.",
    "specs": [
      {
        "label": "Categoría",
        "value": "Novedad Exclusiva"
      }
    ],
    "rating": 4.8,
    "reviews": 25,
    "colors": [
      "#C0C0C0"
    ],
    "stock": 15,
    "tags": [],
    "fragrances": [],
    "isNew": true,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "taza-winter-coming-pq",
    "name": "Taza Winter Coming Pq",
    "slug": "taza-winter-coming-pq",
    "category": "novedades",
    "price": 45.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "ÉPICO",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1/goshopping/novedades/taza_winter_coming_pq.webp"
    ],
    "description": "Descubre esta increíble pieza de nuestra colección de Novedades. Un artículo con detalles únicos que destaca en cualquier colección o ambiente.",
    "specs": [
      {
        "label": "Categoría",
        "value": "Novedad Exclusiva"
      }
    ],
    "rating": 4.8,
    "reviews": 25,
    "colors": [
      "#C0C0C0"
    ],
    "stock": 15,
    "tags": [],
    "fragrances": [],
    "isNew": true,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "tulipanes-infinito-octogonal",
    "name": "Tulipanes Infinito Octogonal",
    "slug": "tulipanes-infinito-octogonal",
    "category": "novedades",
    "price": 34.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "ROMÁNTICO",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774224722/goshopping/novedades/tulipanes_infinito_octogonal.webp"
    ],
    "description": "Descubre esta increíble pieza de nuestra colección de Novedades. Un artículo con detalles únicos que destaca en cualquier colección o ambiente.",
    "specs": [
      {
        "label": "Categoría",
        "value": "Novedad Exclusiva"
      }
    ],
    "rating": 4.8,
    "reviews": 25,
    "colors": [
      "#C0C0C0"
    ],
    "stock": 15,
    "tags": [],
    "fragrances": [],
    "isNew": true,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "tulipanes-infinito-12",
    "name": "Tulipanes Infinito 12",
    "slug": "tulipanes-infinito-12",
    "category": "novedades",
    "price": 24.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "ROMÁNTICO",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1/goshopping/novedades/tulipanes_infinito_12.webp"
    ],
    "description": "Descubre esta increíble pieza de nuestra colección de Novedades. Un artículo con detalles únicos que destaca en cualquier colección o ambiente.",
    "specs": [
      {
        "label": "Categoría",
        "value": "Novedad Exclusiva"
      }
    ],
    "rating": 4.8,
    "reviews": 25,
    "colors": [
      "#C0C0C0"
    ],
    "stock": 15,
    "tags": [],
    "fragrances": [],
    "isNew": true,
    "isOnSale": false,
    "salePercent": null
  },
  //********************************************************************//
  //           QUEMADORES - CASCADAS DE AGUA
  //********************************************************************//
  {
    "id": "quemador-zen-38",
    "name": "Quemador Clarity 38",
    "slug": "quemador-zen-38",
    "category": "quemadores",
    "price": 32.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "CLARIDAD",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1/goshopping/quemadores/quemador_7253_38.webp"
    ],
    "description": "Despeja tu mente de pensamientos pesados. Su forma abierta y clara invita a la transparencia mental y a la frescura de nuevos comienzos.",
    "specs": [
      {
        "label": "Propósito",
        "value": "Enfoque Mental"
      }
    ],
    "rating": 4.5,
    "reviews": 27,
    "colors": [
      "------"
    ],
    "stock": 30,
    "tags": [],
    "fragrances": [],
    "isNew": false,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "quemador-zen-42",
    "name": "Quemador Peace 42",
    "slug": "quemador-zen-42",
    "category": "quemadores",
    "price": 32.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "PAZ",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1/goshopping/quemadores/quemador_7253_42.webp"
    ],
    "description": "Un monumento a la tranquilidad. Este quemador de gran tamaño es una pieza de declaración que trae paz inmediata a cualquier sala o dormitorio.",
    "specs": [
      {
        "label": "Atributo",
        "value": "Pieza de Lujo"
      }
    ],
    "rating": 5,
    "reviews": 18,
    "colors": [
      "---------"
    ],
    "stock": 10,
    "tags": [],
    "fragrances": [],
    "isNew": true,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "cascada-agua-aves",
    "name": "Cascada de Agua Aves Paraíso",
    "slug": "cascada-agua-aves",
    "category": "quemadores",
    "price": 89.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "EXCLUSIVO",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1/goshopping/quemadores/cascada_agua_aves.webp"
    ],
    "description": "Un santuario de paz en tu hogar. Las aves del paraíso guían el humo en una cascada hipnótica que relaja los sentidos y purifica el ambiente con elegancia artística.",
    "specs": [
      {
        "label": "Diseño",
        "value": "Aves del Paraíso"
      }
    ],
    "rating": 4.9,
    "reviews": 45,
    "colors": [
      "-----------"
    ],
    "stock": 12,
    "tags": [],
    "fragrances": [],
    "isNew": true,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "cascada-agua-elefante-sagrado",
    "name": "Cascada de Agua Elefante Sagrado",
    "slug": "cascada-agua-elefante-sagrado",
    "category": "quemadores",
    "price": 88.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "FORTUNA",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1/goshopping/quemadores/cascada_agua_elef.webp"
    ],
    "description": "Atrae la buena suerte y la sabiduría. Este majestuoso elefante sagrado crea un flujo de humo zen que simboliza la abundancia y la paz espiritual en cada rincón.",
    "specs": [
      {
        "label": "Símbolo",
        "value": "Sabiduría y Suerte"
      }
    ],
    "rating": 4.8,
    "reviews": 112,
    "colors": [
      "---------"
    ],
    "stock": 20,
    "tags": [],
    "fragrances": [],
    "isNew": false,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "quemador-zen-28",
    "name": "Quemador Zen Circle 28",
    "slug": "quemador-zen-28",
    "category": "quemadores",
    "price": 31.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "ZEN",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1/goshopping/quemadores/quemador_7253_28.webp"
    ],
    "description": "Minimalismo terapéutico. Un diseño circular que invita a la introspección y el equilibrio, permitiendo que el incienso fluya en una danza de calma absoluta.",
    "specs": [
      {
        "label": "Estilo",
        "value": "Minimalista"
      }
    ],
    "rating": 4.7,
    "reviews": 34,
    "colors": [
      "-------"
    ],
    "stock": 25,
    "tags": [],
    "fragrances": [],
    "isNew": true,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "quemador-zen-29",
    "name": "Quemador Loto Mystic 29",
    "slug": "quemador-zen-29",
    "category": "quemadores",
    "price": 32.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "MÍSTICO",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1/goshopping/quemadores/quemador_7253_29.webp"
    ],
    "description": "La pureza del loto en tu meditación. Este diseño místico captura la esencia de la flor sagrada, creando un ambiente ideal para la relajación profunda.",
    "specs": [
      {
        "label": "Motivo",
        "value": "Flor de Loto"
      }
    ],
    "rating": 4.8,
    "reviews": 28,
    "colors": [
      "--------"
    ],
    "stock": 18,
    "tags": [],
    "fragrances": [],
    "isNew": false,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "quemador-zen-30",
    "name": "Quemador Harmony 30",
    "slug": "quemador-zen-30",
    "category": "quemadores",
    "price": 31.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "ARMONÍA",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1/goshopping/quemadores/quemador_7253_30.webp"
    ],
    "description": "Encuentra tu centro energético. El quemador Harmony combina líneas suaves y fluidas para una experiencia de aromaterapia que restaura la paz interior.",
    "specs": [
      {
        "label": "Efecto",
        "value": "Armonizador"
      }
    ],
    "rating": 4.6,
    "reviews": 41,
    "colors": [
      "------"
    ],
    "stock": 22,
    "tags": [],
    "fragrances": [],
    "isNew": false,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "quemador-zen-33",
    "name": "Quemador Flow 33",
    "slug": "quemador-zen-33",
    "category": "quemadores",
    "price": 31.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "FLUIDEZ",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1/goshopping/quemadores/quemador_7253_33.webp"
    ],
    "description": "Deja que la energía fluya sin obstáculos. Su diseño aerodinámico permite una caída de humo espectacular, perfecta para hipnotizar el estrés diario.",
    "specs": [
      {
        "label": "Diseño",
        "value": "Aerodinámico"
      }
    ],
    "rating": 4.9,
    "reviews": 56,
    "colors": [
      "-------"
    ],
    "stock": 15,
    "tags": [],
    "fragrances": [],
    "isNew": true,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "quemador-zen-34",
    "name": "Quemador Balance 34",
    "slug": "quemador-zen-34",
    "category": "quemadores",
    "price": 31.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "EQUILIBRIO",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1/goshopping/quemadores/quemador_7253_34.webp"
    ],
    "description": "El equilibrio perfecto entre arte y función. Una pieza sólida que ancla la energía de tus espacios mientras desprende tus aromas favoritos.",
    "specs": [
      {
        "label": "Material",
        "value": "Cerámica Reforzada"
      }
    ],
    "rating": 4.7,
    "reviews": 23,
    "colors": [
      "--------"
    ],
    "stock": 19,
    "tags": [],
    "fragrances": [],
    "isNew": false,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "quemador-zen-37",
    "name": "Quemador Serenity 37",
    "slug": "quemador-zen-37",
    "category": "quemadores",
    "price": 32.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "SERENIDAD",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1/goshopping/quemadores/quemador_7253_37.webp"
    ],
    "description": "Silencio visual que perfuma. Serenity es la elección ideal para rincones de lectura o altares personales donde se busca la calma absoluta.",
    "specs": [
      {
        "label": "Uso",
        "value": "Meditación/Lectura"
      }
    ],
    "rating": 4.8,
    "reviews": 39,
    "colors": [
      "-------"
    ],
    "stock": 24,
    "tags": [],
    "fragrances": [],
    "isNew": true,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "quemador-modern-24",
    "name": "Quemador Modern Art 24",
    "slug": "quemador-modern-24",
    "category": "quemadores",
    "price": 49.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "MODERNO",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1/goshopping/quemadores/quemador_7677_24.webp"
    ],
    "description": " Vanguardia en el arte del incienso. Una reinterpretación moderna de los quemadores clásicos que encaja perfectamente en lofts y oficinas contemporáneas.",
    "specs": [
      {
        "label": "Estilo",
        "value": "Contemporáneo"
      }
    ],
    "rating": 4.7,
    "reviews": 31,
    "colors": [
      "-------"
    ],
    "stock": 15,
    "tags": [],
    "fragrances": [],
    "isNew": false,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "quemador-minimal-42",
    "name": "Quemador Minimalist 7677-42",
    "slug": "quemador-minimal-42",
    "category": "quemadores",
    "price": 31.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "ESSENCIAL",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1/goshopping/quemadores/quemador_7677_42.webp"
    ],
    "description": "Menos es más. El Minimalist 42 ofrece la forma más pura de disfrutar tus fragancias favoritas sin distracciones visuales, solo esencia pura.",
    "specs": [
      {
        "label": "Filosofía",
        "value": "Minimalismo Puro"
      }
    ],
    "rating": 4.6,
    "reviews": 22,
    "colors": [
      "-------"
    ],
    "stock": 35,
    "tags": [],
    "fragrances": [],
    "isNew": true,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "quemador-angel-rgb",
    "name": "Quemador Ángel de Luz RGB",
    "slug": "quemador-angel-rgb",
    "category": "quemadores",
    "price": 83.59,
    "oldPrice": 149,
    "currency": "PEN",
    "badge": "MÁGICO",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1/goshopping/quemadores/quemador_angel_rgb.webp"
    ],
    "description": "Una bendición luminosa para tus sentidos. Este ángel no solo quema tu incienso favorito, sino que ilumina tu espacio con una luz RGB suave y celestial.",
    "specs": [
      {
        "label": "Luz",
        "value": "RGB Multicolor"
      },
      {
        "label": "Efecto",
        "value": "Celestial"
      }
    ],
    "rating": 5,
    "reviews": 64,
    "colors": [
      "--------"
    ],
    "stock": 8,
    "tags": [],
    "fragrances": [],
    "isNew": true,
    "isOnSale": true,
    "salePercent": 20
  },
  {
    "id": "quemador-caidas",
    "name": "Quemador Futurista Capas",
    "slug": "quemador-caidas",
    "category": "quemadores",
    "price": 49.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "TECH",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1/goshopping/quemadores/quemador_bot.webp"
    ],
    "description": "El futuro de la aromaterapia tradicional. Un diseño robótico audaz que fusiona la tecnología visual con el ritual milenario del incienso.",
    "specs": [
      {
        "label": "Estilo",
        "value": "Futurista/Tech"
      }
    ],
    "rating": 4.8,
    "reviews": 25,
    "colors": [
      "---------"
    ],
    "stock": 14,
    "tags": [],
    "fragrances": [],
    "isNew": true,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "quemador-castillo-dragon",
    "name": "Quemador Castillo del Dragón",
    "slug": "quemador-castillo-dragon",
    "category": "quemadores",
    "price": 69.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "ÉPICO",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1/goshopping/quemadores/quemador_castillo_dragon.webp"
    ],
    "description": "Domina el reino del aroma. Una fortaleza épica donde el dragón custodia el fuego sagrado, creando una escena de fantasía con cada nube de humo.",
    "specs": [
      {
        "label": "Temática",
        "value": "Fantasía Épica"
      },
      {
        "label": "Detalle",
        "value": "Alta Precisión"
      }
    ],
    "rating": 4.9,
    "reviews": 87,
    "colors": [
      "--------"
    ],
    "stock": 7,
    "tags": [],
    "fragrances": [],
    "isNew": false,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "quemador-dragon-cascada",
    "name": "Quemador Dragón de Cascada",
    "slug": "quemador-dragon-cascada",
    "category": "quemadores",
    "price": 59.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "TOP VENTAS",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1/goshopping/quemadores/quemador_dragon.webp"
    ],
    "description": "La esencia del poder. Mira cómo el humo emula el aliento de un dragón fluyendo hacia abajo en una cascada mística que cautiva a todos los presentes.",
    "specs": [
      {
        "label": "Tipo",
        "value": "Cascada de Humo"
      }
    ],
    "rating": 4.9,
    "reviews": 265,
    "colors": [
      "-------"
    ],
    "stock": 20,
    "tags": [],
    "fragrances": [],
    "isNew": false,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "quemador-dragon-roca",
    "name": "Quemador Dragón sobre Roca",
    "slug": "quemador-dragon-roca",
    "category": "quemadores",
    "price": 62.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "SALVAJE",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1/goshopping/quemadores/quemador_dragon_roca.webp"
    ],
    "description": "La fuerza de la naturaleza en casa. El dragón vigila desde su peñasco rocoso mientras el aroma se expande en una nube mágica y protectora.",
    "specs": [
      {
        "label": "Base",
        "value": "Efecto Roca"
      }
    ],
    "rating": 4.7,
    "reviews": 31,
    "colors": [
      "--------"
    ],
    "stock": 16,
    "tags": [],
    "fragrances": [],
    "isNew": false,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "quemador-elefante-vela",
    "name": "Quemador Elefante Porta Vela",
    "slug": "quemador-elefante-vela",
    "category": "quemadores",
    "price": 25.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "CÁLIDO",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1/goshopping/quemadores/quemador_elef_vela.webp"
    ],
    "description": "Doble funcionalidad, doble paz. Un tierno elefante que sostiene tu incienso y una vela pequeña, creando una atmósfera de calidez inigualable.",
    "specs": [
      {
        "label": "Extra",
        "value": "Soporte para Vela"
      }
    ],
    "rating": 4.6,
    "reviews": 93,
    "colors": [
      "-------"
    ],
    "stock": 25,
    "tags": [],
    "fragrances": [],
    "isNew": false,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "quemador-elefante-sabio",
    "name": "Quemador Elefante Sabio",
    "slug": "quemador-elefante-sabio",
    "category": "quemadores",
    "price": 59.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "ZEN",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1/goshopping/quemadores/quemador_elefante.webp"
    ],
    "description": "Serenidad en cada inhalación. Deja que la sabiduría del elefante purifique tus pensamientos a través del humo sagrado de tus fragancias favoritas.",
    "specs": [
      {
        "label": "Material",
        "value": "Cerámica"
      }
    ],
    "rating": 4.8,
    "reviews": 75,
    "colors": [
      "--------"
    ],
    "stock": 30,
    "tags": [],
    "fragrances": [],
    "isNew": false,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "quemador-te-zen",
    "name": "Quemador Té y Meditación",
    "slug": "quemador-te-zen",
    "category": "quemadores",
    "price": 48.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "RELAX",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1/goshopping/quemadores/quemador_en_te.webp"
    ],
    "description": "El ritual del té transformado. Un diseño que evoca la ceremonia del té, ideal para acompañar tus momentos de lectura y relax con un aroma envolvente.",
    "specs": [
      {
        "label": "Inspiración",
        "value": "Ceremonia del Té"
      }
    ],
    "rating": 4.7,
    "reviews": 48,
    "colors": [
      "--------"
    ],
    "stock": 22,
    "tags": [],
    "fragrances": [],
    "isNew": true,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "quemador-carpa-koi",
    "name": "Quemador Pez Carpa Koi",
    "slug": "quemador-carpa-koi",
    "category": "quemadores",
    "price": 49.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "PROSPERIDAD",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1/goshopping/quemadores/quemador_fish.webp"
    ],
    "description": "Fluidez y éxito en tu vida. Se dice que el pez Koi atrae la abundancia; este quemador es un recordatorio visual de la perseverancia y el flujo positivo.",
    "specs": [
      {
        "label": "Significado",
        "value": "Abundancia"
      }
    ],
    "rating": 4.9,
    "reviews": 52,
    "colors": [
      "------------"
    ],
    "stock": 15,
    "tags": [],
    "fragrances": [],
    "isNew": false,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "quemador-loto-divino",
    "name": "Quemador Flor de Loto Divina",
    "slug": "quemador-loto-divino",
    "category": "quemadores",
    "price": 79.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "SAGRADO",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774224751/goshopping/quemadores/quemador_flor_de_loto2.webp"
    ],
    "description": "Símbolo de renacimiento ypureza. La flor de loto se abre paso entre el humo para recordarte que la belleza puede florecer en cualquier circunstancia.",
    "specs": [
      {
        "label": "Motivo",
        "value": "Flor de Loto"
      }
    ],
    "rating": 4.8,
    "reviews": 67,
    "colors": [
      "---------"
    ],
    "stock": 40,
    "tags": [],
    "fragrances": [],
    "isNew": false,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "quemador-loto-pure",
    "name": "Quemador Flor de Loto Pura",
    "slug": "quemador-loto-pure",
    "category": "quemadores",
    "price": 79.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "ESENCIAL",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1/goshopping/quemadores/quemador_flor_loto1.webp"
    ],
    "description": "Belleza natural y simplificada. Un diseño de loto de líneas puras que aporta una nota de frescura floral y paz a cualquier rincón de tu hogar.",
    "specs": [
      {
        "label": "Diseño",
        "value": "Loto Abierto"
      }
    ],
    "rating": 4.6,
    "reviews": 89,
    "colors": [
      "--------"
    ],
    "stock": 50,
    "tags": [],
    "fragrances": [],
    "isNew": true,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "quemador-hojas-otono",
    "name": "Quemador Hojas de Otoño",
    "slug": "quemador-hojas-otono",
    "category": "quemadores",
    "price": 31.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "NATURA",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1/goshopping/quemadores/quemador_hojas.webp"
    ],
    "description": "La calma del bosque en casa. Siente la brisa de otoño con este quemador que captura la delicadeza de las hojas caídas en una composición orgánica.",
    "specs": [
      {
        "label": "Estilo",
        "value": "Nature Inspired"
      }
    ],
    "rating": 4.5,
    "reviews": 76,
    "colors": [
      "-----------"
    ],
    "stock": 35,
    "tags": [],
    "fragrances": [],
    "isNew": false,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "quemador-leon-majestuoso",
    "name": "Quemador León Majestuoso",
    "slug": "quemador-leon-majestuoso",
    "category": "quemadores",
    "price": 52.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "PODER",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1/goshopping/quemadores/quemador_leon.webp"
    ],
    "description": "Protector del hogar y símbolo de coraje. El león imperial custodia tus espacios mientras el humo sagrado disipa cualquier sombra de negatividad.",
    "specs": [
      {
        "label": "Símbolo",
        "value": "Coraje y Fuerza"
      }
    ],
    "rating": 4.9,
    "reviews": 31,
    "colors": [
      "---------"
    ],
    "stock": 11,
    "tags": [],
    "fragrances": [],
    "isNew": true,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "quemador-lotus-koi",
    "name": "Quemador Flor de Loto y Pez",
    "slug": "quemador-lotus-koi",
    "category": "quemadores",
    "price": 45.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "EQUILIBRIO",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1/goshopping/quemadores/quemador_lotus_fish.webp"
    ],
    "description": "La simbiosis perfecta del estanque zen. La carpa Koi y el loto se funden en esta pieza para traerte serenidad, equilibrio y vitalidad constante.",
    "specs": [
      {
        "label": "Diseño",
        "value": "Loto y Pez Koi"
      }
    ],
    "rating": 4.8,
    "reviews": 44,
    "colors": [
      "-------"
    ],
    "stock": 18,
    "tags": [],
    "fragrances": [],
    "isNew": false,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "quemador-manzana-suerte",
    "name": "Quemador Manzana de la Suerte",
    "slug": "quemador-manzana-suerte",
    "category": "quemadores",
    "price": 65.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "DULCE",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1/goshopping/quemadores/quemador_manzana.webp"
    ],
    "description": "Atrae la salud y la vitalidad. Un diseño frutal encantador que simboliza la abundancia y la renovación constante de la energía positiva.",
    "specs": [
      {
        "label": "Forma",
        "value": "Manzana Orgánica"
      }
    ],
    "rating": 4.6,
    "reviews": 58,
    "colors": [
      "---------"
    ],
    "stock": 26,
    "tags": [],
    "fragrances": [],
    "isNew": true,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "quemador-mujer-zen",
    "name": "Quemador Silueta Femenina Zen",
    "slug": "quemador-mujer-zen",
    "category": "quemadores",
    "price": 49.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "ELEGANCIA",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1/goshopping/quemadores/quemador_mujer.webp"
    ],
    "description": "La gracia y la calma en una pieza. Esta silueta femenina en meditación irradia paz y sofisticación, siendo el centro de atención de cualquier altar personal.",
    "specs": [
      {
        "label": "Estilo",
        "value": "Escultórico"
      }
    ],
    "rating": 4.9,
    "reviews": 29,
    "colors": [
      "--------"
    ],
    "stock": 13,
    "tags": [],
    "fragrances": [],
    "isNew": true,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "quemador-oracion-calma",
    "name": "Quemador Oración y Calma",
    "slug": "quemador-oracion-calma",
    "category": "quemadores",
    "price": 59.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "REZO",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1/goshopping/quemadores/quemador_ora.webp"
    ],
    "description": "Sintoniza con lo divino. Un diseño que invita a la plegaria y la gratitud diaria, creando un ambiente de respeto y profunda conexión espiritual.",
    "specs": [
      {
        "label": "Uso",
        "value": "Espiritual/Oración"
      }
    ],
    "rating": 4.8,
    "reviews": 37,
    "colors": [
      "---------"
    ],
    "stock": 18,
    "tags": [],
    "fragrances": [],
    "isNew": false,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "quemador-pez-abundancia",
    "name": "Quemador Pez de la Abundancia",
    "slug": "quemador-pez-abundancia",
    "category": "quemadores",
    "price": 49.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "SUERTE",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1/goshopping/quemadores/quemador_pez.webp"
    ],
    "description": "Navega en aguas de prosperidad. El pez de la abundancia te acompaña en tus momentos de enfoque, asegurando que el flujo de buena fortuna no se detenga.",
    "specs": [
      {
        "label": "Significado",
        "value": "Prosperidad"
      }
    ],
    "rating": 4.7,
    "reviews": 41,
    "colors": [
      "---------"
    ],
    "stock": 21,
    "tags": [],
    "fragrances": [],
    "isNew": false,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "quemador-shiva",
    "name": "Quemador Shiva Meditación",
    "slug": "quemador-shiva",
    "category": "quemadores",
    "price": 71.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "POTENCIA",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1/goshopping/quemadores/quemador_shiv.webp"
    ],
    "description": "La fuerza del cambio positivo. Shiva en meditación te brinda la determinación necesaria para transformar tu entorno y elevar tu vibración personal.",
    "specs": [
      {
        "label": "Deidad",
        "value": "Shiva"
      }
    ],
    "rating": 5,
    "reviews": 54,
    "colors": [
      "----------"
    ],
    "stock": 15,
    "tags": [],
    "fragrances": [],
    "isNew": true,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "quemador-vase-antique",
    "name": "Quemador Jarrón Antiguo",
    "slug": "quemador-vase-antique",
    "category": "quemadores",
    "price": 25.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "CLÁSICO",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1/goshopping/quemadores/quemador_vase.webp"
    ],
    "description": "Nostalgia que perfuma. Inspirado en ánforas antiguas, este quemador aporta un toque histórico y rústico que embellece cualquier espacio tradicional.",
    "specs": [
      {
        "label": "Estilo",
        "value": "Clásico/Antiguo"
      }
    ],
    "rating": 4.5,
    "reviews": 32,
    "colors": [
      "---------"
    ],
    "stock": 28,
    "tags": [],
    "fragrances": [],
    "isNew": false,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "quemador-vela-flor",
    "name": "Quemador Flor y Vela Artística",
    "slug": "quemador-vela-flor",
    "category": "quemadores",
    "price": 14.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "ART",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1/goshopping/quemadores/quemador_vela_flor.webp"
    ],
    "description": "Arte que ilumina y calma. Una composición floral que integra un espacio para vela, ideal para crear centros de mesa románticos y aromáticos.",
    "specs": [
      {
        "label": "Uso",
        "value": "Decorativo/Aromático"
      }
    ],
    "rating": 4.7,
    "reviews": 29,
    "colors": [
      ""
    ],
    "stock": 24,
    "tags": [],
    "fragrances": [],
    "isNew": true,
    "isOnSale": false,
    "salePercent": null
  },
  //********************************************************************//
  //           TECNOLOGIA
  //********************************************************************//
  {
    "id": "audifonos-m45",
    "name": "Audifonos M45",
    "slug": "audifonos-45",
    "category": "tecnologia",
    "price": 39.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "NUEVO",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1/goshopping/tecnologia/audifono_c45.webp"
    ],
    "description": "El sonido perfecto para escuchar tus Musicas y diseño moderno.",
    "specs": [
      {
        "label": "Diseño",
        "value": "Portatil"
      }
    ],
    "rating": 4.6,
    "reviews": 18,
    "colors": [
      ""
    ],
    "stock": 25,
    "tags": [],
    "fragrances": [],
    "isNew": true,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "lampara-luna",
    "name": "Lampara Luna Aura RGB",
    "slug": "lampara-luna",
    "category": "tecnologia",
    "price": 29.9,
    "oldPrice": 88.9,
    "currency": "PEN",
    "badge": "TOP",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1/goshopping/tecnologia/lampara_luna_tac_rgb.webp"
    ],
    "description": "Siente el ritmo en cada gota. Mira cómo el agua cobra vida al compás de tus canciones favoritas con un espectáculo de luces RGB que redefine tu experiencia auditiva.",
    "specs": [
      {
        "label": "Conexión",
        "value": "Bluetooth 5.0"
      },
      {
        "label": "Luz",
        "value": "RGB Dinámico"
      }
    ],
    "rating": 4.9,
    "reviews": 156,
    "colors": [
      "#000000",
      "#FFFFFF"
    ],
    "stock": 10,
    "tags": [],
    "fragrances": [],
    "isNew": false,
    "isOnSale": true,
    "salePercent": 17
  },
  {
    "id": "lampara-tac-carga",
    "name": "Lámpara Táctil Inalámbrica",
    "slug": "lampara-tac-carga",
    "category": "tecnologia",
    "price": 79.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "SMART",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1/goshopping/tecnologia/lampara_tac_carga.webp"
    ],
    "description": "Iluminación inteligente sin cables. Esta lámpara LED táctil con carga inalámbrica integrada fusiona funcionalidad y diseño, perfecta para tu escritorio moderno.",
    "specs": [
      {
        "label": "Carga",
        "value": "Inalámbrica Qi"
      },
      {
        "label": "Luz",
        "value": "LED Regulable - Diseño Blanco"
      }
    ],
    "rating": 4.8,
    "reviews": 67,
    "colors": [
      "-----"
    ],
    "stock": 18,
    "tags": [],
    "fragrances": [],
    "isNew": true,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "lampara-tac-carga-neg",
    "name": "Lámpara Táctil Dark Edition",
    "slug": "lampara-tac-carga-neg",
    "category": "tecnologia",
    "price": 79.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "DARK",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1/goshopping/tecnologia/lampara_tac_carga_neg.webp"
    ],
    "description": "Elegancia oscura funcional. La versión black de nuestra lámpara táctil con carga inalámbrica, diseñada para ambientes que exigen sofisticación nocturna.",
    "specs": [
      {
        "label": "Carga",
        "value": "Inalámbrica Qi"
      },
      {
        "label": "Luz",
        "value": "LED Warm - Diseño Negro"
      }
    ],
    "rating": 4.7,
    "reviews": 42,
    "colors": [
      "#000000"
    ],
    "stock": 12,
    "tags": [],
    "fragrances": [],
    "isNew": true,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "lamp-geo-celeste",
    "name": "Lámpara Geométrica Aqua",
    "slug": "lamp-geo-celeste",
    "category": "tecnologia",
    "price": 149.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "TENDENCIA",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1/goshopping/tecnologia/lamp_geo_celeste.webp"
    ],
    "description": "Geometría en tono cielo. Una pieza escultórica con luz ambiental celeste que transforma cualquier espacio en un refugio de calma y diseño contemporáneo.",
    "specs": [
      {
        "label": "Diseño",
        "value": "Geométrico"
      },
      {
        "label": "Color",
        "value": "Celeste"
      }
    ],
    "rating": 4.8,
    "reviews": 34,
    "colors": [
      "-----"
    ],
    "stock": 15,
    "tags": [],
    "fragrances": [],
    "isNew": true,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "lamp-geo-dorado",
    "name": "Lámpara Geométrica Gold Luxe",
    "slug": "lamp-geo-dorado",
    "category": "tecnologia",
    "price": 149.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "LUJO",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1/goshopping/tecnologia/lamp_geo_dorado.webp"
    ],
    "description": "Lujo geométrico dorado. La versión premium de nuestra lámpara geométrica con acabados en oro que irradia elegancia y estatus en cada rincón de tu hogar.",
    "specs": [
      {
        "label": "Diseño",
        "value": "Geométrico"
      },
      {
        "label": "Acabado",
        "value": "Dorado"
      }
    ],
    "rating": 4.9,
    "reviews": 22,
    "colors": [
      "-----"
    ],
    "stock": 8,
    "tags": [],
    "fragrances": [],
    "isNew": true,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "lamp-geo-negro",
    "name": "Lámpara Geométrica Stealth",
    "slug": "lamp-geo-negro",
    "category": "tecnologia",
    "price": 149.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "URBAN",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1/goshopping/tecnologia/lamp_geo_negro.webp"
    ],
    "description": "Geometría urbana en negro mate. Un diseño que se integra en ambientes modernos y oscuros, proyectando luz ambiental con actitud industrial y sofisticada.",
    "specs": [
      {
        "label": "Diseño",
        "value": "Geométrico"
      },
      {
        "label": "Acabado",
        "value": "Negro Mate"
      }
    ],
    "rating": 4.7,
    "reviews": 28,
    "colors": [
      "------"
    ],
    "stock": 20,
    "tags": [],
    "fragrances": [],
    "isNew": false,
    "isOnSale": false,
    "salePercent": null
  },
  {
    "id": "parlante-agua-white",
    "name": "Parlante Liquid Crystal",
    "slug": "parlante-agua-white",
    "category": "tecnologia",
    "price": 79.9,
    "oldPrice": 179.9,
    "currency": "PEN",
    "badge": "BEST SELLER",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1/goshopping/tecnologia/parlante_agua.webp"
    ],
    "description": "Espectáculo visual y sonoro. El agua danza al ritmo de tu música favorita con luces RGB envolventes, creando una experiencia sensorial única e irrepetible.",
    "specs": [
      {
        "label": "Conexión",
        "value": "Bluetooth 5.0"
      },
      {
        "label": "Efecto",
        "value": "Agua + RGB"
      }
    ],
    "rating": 4.9,
    "reviews": 198,
    "colors": [
      "-------"
    ],
    "stock": 15,
    "tags": [],
    "fragrances": [],
    "isNew": false,
    "isOnSale": true,
    "salePercent": 17
  },
  {
    "id": "parlante-agua-black",
    "name": "Parlante Liquid Shadow",
    "slug": "parlante-agua-black",
    "category": "tecnologia",
    "price": 79.9,
    "oldPrice": 189.9,
    "currency": "PEN",
    "badge": "PREMIUM",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1/goshopping/tecnologia/parlante_agua_black.jpeg"
    ],
    "description": "Oscuridad rítmica total. La versión dark de nuestro parlante de agua más vendido, diseñado para quienes buscan potencia sonora con un estilo nocturno único.",
    "specs": [
      {
        "label": "Conexión",
        "value": "Bluetooth 5.0"
      },
      {
        "label": "Efecto",
        "value": "Agua + RGB Dark"
      }
    ],
    "rating": 4.8,
    "reviews": 89,
    "colors": [
      "------"
    ],
    "stock": 10,
    "tags": [],
    "fragrances": [],
    "isNew": true,
    "isOnSale": true,
    "salePercent": 16
  },
  {
    "id": "parlante-medusa",
    "name": "Parlante Medusa Glow",
    "slug": "parlante-medusa",
    "category": "tecnologia",
    "price": 49.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "EXCLUSIVO",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1/goshopping/tecnologia/parlante_medusa.webp"
    ],
    "description": "Hipnótica experiencia de sonido. Una medusa bioluminiscente que flota y brilla al ritmo de tu música, fusionando arte visual con audio premium de alta fidelidad.",
    "specs": [
      {
        "label": "Conexión",
        "value": "Bluetooth 5.0"
      },
      {
        "label": "Efecto",
        "value": "Medusa LED"
      }
    ],
    "rating": 5,
    "reviews": 45,
    "colors": [
      "-----"
    ],
    "stock": 8,
    "tags": [],
    "fragrances": [],
  },
  {
    "id": "proyector-astronauta-luna",
    "name": "Proyector Astronauta Luna",
    "slug": "proyector-astronauta-luna",
    "category": "tecnologia",
    "price": 44.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "EXCLUSIVO",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774224882/goshopping/tecnologia/astronauta__luna_proyector.webp"
    ],
    "description": "Dormir observando las auroras boreales, hara que contemples la calma y tranquilidad de un sueño reparador.",
    "specs": [
      {
        "label": "Conexión Parlante",
        "value": "Bluetooth 5.0"
      },
      {
        "label": "Efecto",
        "value": "Luz RGB suave"
      }
    ],
    "rating": 5,
    "reviews": 45,
    "colors": [
      "-----"
    ],
    "stock": 8,
    "tags": [],
    "fragrances": [],
  },
  {
    "id": "audifono-m25",
    "name": "Audifono M25",
    "slug": "audifono-m25",
    "category": "tecnologia",
    "price": 39.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "EXCLUSIVO",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774224884/goshopping/tecnologia/audifono_c25.webp"
    ],
    "description": "Para aquellos amantes del sonido GAMER. Ideal para sentir al maximo el poder de estar dentro del juego",
    "specs": [
      {
        "label": "Conexión",
        "value": "Bluetooth 5.0"
      },
    ],
    "rating": 5,
    "reviews": 45,
    "colors": [
      "-----"
    ],
    "stock": 8,
    "tags": [],
    "fragrances": [],
  },
  {
    "id": "astronauta-galaxia",
    "name": "Astronauta Galaxia",
    "slug": "astronauta-galaxia",
    "category": "tecnologia",
    "price": 49.9,
    "oldPrice": null,
    "currency": "PEN",
    "badge": "EXCLUSIVO",
    "badgeColor": null,
    "images": [
      "https://res.cloudinary.com/dod8hhjoo/image/upload/v1774224883/goshopping/tecnologia/astronauta_proyector.webp"
    ],
    "description": "Hipnótica experiencia de ver las estrellas y galaxias antes de un placentero sueño reparador.",
    "specs": [
      {
        "label": "Parlante Conexión",
        "value": "Bluetooth 5.0"
      },
      {
        "label": "Efecto",
        "value": "Galaxia LED"
      }
    ],
    "rating": 5,
    "reviews": 45,
    "colors": [
      "-----"
    ],
    "stock": 8,
    "tags": [],
    "fragrances": [],
  },
  
];